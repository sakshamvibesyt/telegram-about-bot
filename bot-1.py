...
async def admin_command(update, context):
    if not owner_only(update):
        ...
        return
    await update.effective_message.reply_text(admin_dashboard_text(), reply_markup=admin_menu())


async def admin_input(update, context):
    state = context.user_data.get("admin_input")
    if not state or not owner_only(update) or not update.effective_message:
        return False
    ...
    if state == "promo_text":
        ...
        context.user_data.pop("admin_input", None)
        await update.effective_message.reply_text("✅ Promo message updated.", reply_markup=promo_admin_menu())
        return True
    if state.startswith("add_link:"):
        parts = value.split("|", 1)
        if len(parts) != 2 or not parts[0].strip() or not parts[1].strip().startswith(("https://", "http://")):
            await update.effective_message.reply_text("❌ Format: Button Name | https://example.com")
            return True
        with db_connect() as conn:
        ...
        context.user_data.pop("admin_input", None)
        await update.effective_message.reply_text("✅ Link added.", reply_markup=links_admin_menu())
        return True
    if state.startswith("edit_link:"):
        ...
        context.user_data.pop("admin_input", None)
        await update.effective_message.reply_text("✅ Link updated.", reply_markup=links_admin_menu())
        return True
    if state in ("coin_add", "coin_remove"):
        parts = value.split()
        if len(parts) != 1 or not parts[0].lstrip("+").isdigit():
            await update.effective_message.reply_text("❌ Sirf amount bhejo, example: 500")
            return True
        amount = int(parts[0])
        if amount <= 0:
            await update.effective_message.reply_text("❌ Amount 0 se zyada hona chahiye.")
            return True
        target_msg = update.effective_message.reply_to_message
        target = target_msg.from_user if target_msg and target_msg.from_user else None
        if not target or target.is_bot:
            await update.effective_message.reply_text("❌ Jis member ko coins dene hain, uske message ko reply karke amount bhejo.")
            return True
        ...
        if state == "coin_remove" and amount > current:
            await update.effective_message.reply_text(f"❌ Itne coins available nahi hain. Current balance: {current}")
            return True
    ...
    if state == "redeem_generate":
        ...
        if len(parts) != 2 or not parts[0].isdigit() or not parts[1].isdigit():
            await update.effective_message.reply_text("❌ Format: AMOUNT COUNT\nExample: 1000 50")
            return True
        amount, count = int(parts[0]), int(parts[1])
        if amount <= 0 or count <= 0 or count > 500:
            await update.effective_message.reply_text("❌ Amount > 0 aur count 1-500 ke beech rakho.")
            return True
        codes = []
    ...
    if state == "mention_text":
        ...
        context.user_data.pop("admin_input", None)
        await update.effective_message.reply_text("✅ Custom mention message saved.", reply_markup=mention_admin_menu())
        return True
    if state == "mention_button":
        ...
        context.user_data.pop("admin_input", None)
        await update.effective_message.reply_text("✅ Custom button saved.", reply_markup=mention_admin_menu())
        return True
    return False
...
async def mod_command(update, context):
    if not owner_only(update):
        await update.effective_message.reply_text("❌ Access denied.")
        return
    await update.effective_message.reply_text("🛡️ MODERATION\n\nUse /warn, /mute, /unmute, /ban or /unban as a reply to a user's message.", reply_markup=moderation_menu())


def replied_user(update):
    msg = update.effective_message.reply_to_message if update.effective_message else None
...
async def warn_user(update, context):
    ...
    if not user:
        await update.effective_message.reply_text("⚠️ Reply to the user's message and use /warn."); return
    with db_connect() as conn:
    ...
    await update.effective_message.reply_text(f"⚠️ Warning given to {user.full_name}. Total warnings: {count}")
...
async def mute_user(update, context):
    ...
    user = replied_user(update)
    if not user:
        await update.effective_message.reply_text("⚠️ Reply to the user's message and use /mute."); return
    try:
        ...
        await update.effective_message.reply_text(f"🔇 {user.full_name} muted.")
    except Exception as e:
        await update.effective_message.reply_text(f"❌ Mute failed: {e}")
...
async def unmute_user(update, context):
    ...
    user = replied_user(update)
    if not user:
        await update.effective_message.reply_text("⚠️ Reply to the user's message and use /unmute."); return
    try:
        from telegram import ChatPermissions
        ...
        await update.effective_message.reply_text(f"🔊 {user.full_name} unmuted.")
    except Exception as e:
        await update.effective_message.reply_text(f"❌ Unmute failed: {e}")
...
async def ban_user(update, context):
    ...
    user = replied_user(update)
    if not user:
        await update.effective_message.reply_text("⚠️ Reply to the user's message and use /ban."); return
    try:
        await context.bot.ban_chat_member(update.effective_chat.id, user.id)
        await update.effective_message.reply_text(f"🚫 {user.full_name} banned.")
    except Exception as e:
...
async def unban_user(update, context):
    ...
    user = replied_user(update)
    if not user:
        await update.effective_message.reply_text("⚠️ Reply to the user's message and use /unban."); return
    try:
        await context.bot.unban_chat_member(update.effective_chat.id, user.id, only_if_banned=True)
        await update.effective_message.reply_text(f"✅ {user.full_name} unbanned.")
    except Exception as e:
...
async def test_greet_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not chat or chat.type not in ("group", "supergroup"):
        await update.effective_message.reply_text("⚠️ /testgreet sirf group mein use karo.")
        return
    ...
    try:
        ...
        if member.status not in ("creator", "administrator") and user.id != OWNER_ID:
            await update.effective_message.reply_text("❌ Sirf group admins /testgreet use kar sakte hain.")
            return
    except Exception:
        if user.id != OWNER_ID:
            await update.effective_message.reply_text("❌ Admin permission verify nahi ho saki.")
            return
    ...
    await update.effective_message.reply_text("🧪 Test greeting bhej raha hoon...")
...
def init_db():
    with db_connect() as conn:
        ...
        conn.execute("CREATE TABLE IF NOT EXISTS inventories (chat_id INTEGER NOT NULL, user_id INTEGER NOT NULL, item_id INTEGER NOT NULL, item_name TEXT NOT NULL, rarity TEXT NOT NULL DEFAULT 'RARE', quantity INTEGER NOT NULL DEFAULT 1, acquired_at TEXT NOT NULL, PRIMARY KEY(chat_id,user_id,item_id))")
        ...
        shop_defaults = [
            ...
            ("🥇 GOLD BAR", "Virtual collectible • Gold tier", 5000, "collectible:LEGENDARY", 1),
            ("💎 DIAMOND", "Virtual collectible • Diamond tier", 10000, "collectible:MYTHIC", 1),
            ("❤️ RUBY", "Virtual collectible • Ruby tier", 15000, "collectible:EPIC", 1),
            ("💚 EMERALD", "Virtual collectible • Emerald tier", 20000, "collectible:EPIC", 1),
            ("💙 SAPPHIRE", "Virtual collectible • Sapphire tier", 30000, "collectible:RARE", 1),
            ("⚪ PLATINUM BAR", "Virtual collectible • Platinum tier", 50000, "collectible:ULTRA", 1),
            ("👑 ROYAL CROWN", "Virtual collectible • Royal tier", 75000, "collectible:MYTHIC", 1),
            ("🔥 PHOENIX EMBLEM", "Virtual collectible • Phoenix tier", 100000, "collectible:LEGENDARY", 1),
        ]
...
async def elite_command(update, context):
    ...
    user = update.effective_user
    if not chat or chat.type not in ("group", "supergroup"):
        await update.effective_message.reply_text("⚠️ /elite sirf group mein use karo.")
        return
    status = "ELITE ACTIVE 💎" if is_elite(chat.id, user.id) else "NOT ACTIVE ❌"
    await update.effective_message.reply_text(f"💎 𝐄𝐋𝐈𝐓𝐄 𝐒𝐓𝐀𝐓𝐔𝐒\n\n👤 {user.full_name}\n✨ Status: {status}")
...
async def giveelite_command(update, context):
    ...
    if not chat or chat.type not in ("group", "supergroup") or not target or target.is_bot:
        await update.effective_message.reply_text("💎 Member ke message ko reply karke /giveelite use karo.")
        return
    grant_elite(chat.id, target.id)
    await update.effective_message.reply_text(f"💎 Elite badge granted to {target.full_name}!\n✨ /elite se status check kar sakte hain.")
...
async def removeelite_command(update, context):
    ...
    target = replied_user(update)
    if not chat or chat.type not in ("group", "supergroup") or not target or target.is_bot:
        await update.effective_message.reply_text("💎 Member ke message ko reply karke /removeelite use karo.")
        return
    removed = remove_elite(chat.id, target.id)
    await update.effective_message.reply_text((f"❌ Elite badge removed from {target.full_name}." if removed else f"ℹ️ {target.full_name} ke paas Elite active nahi tha."))
...
async def vip_command(update, context):
    ...
    user = update.effective_user
    if not chat or chat.type not in ("group", "supergroup"):
        await update.effective_message.reply_text("⚠️ /vip sirf group mein use karo.")
        return
    status = "ACTIVE 👑" if is_vip(chat.id, user.id) else "NOT ACTIVE ❌"
    await update.effective_message.reply_text(f"👑 𝐕𝐈𝐏 𝐒𝐓𝐀𝐓𝐔𝐒\n\n👤 {user.full_name}\n✨ Status: {status}")
...
async def givevip_command(update, context):
    ...
    target = replied_user(update)
    if not chat or chat.type not in ("group", "supergroup") or not target or target.is_bot:
        await update.effective_message.reply_text("👑 Member ke message ko reply karke /givevip use karo.")
        return
    grant_vip(chat.id, target.id)
    await update.effective_message.reply_text(f"👑 VIP badge granted to {target.full_name}!\n✨ /vip se status check kar sakte hain.")
...
async def removevip_command(update, context):
    ...
    target = replied_user(update)
    if not chat or chat.type not in ("group", "supergroup") or not target or target.is_bot:
        await update.effective_message.reply_text("👑 Member ke message ko reply karke /removevip use karo.")
        return
    removed = remove_vip(chat.id, target.id)
    await update.effective_message.reply_text((f"❌ VIP badge removed from {target.full_name}." if removed else f"ℹ️ {target.full_name} ke paas VIP active nahi tha."))
...
def rarity_for_item(name, reward_type):
    if reward_type.startswith("collectible:"):
        return reward_type.split(":", 1)[1]
    if "ELITE" in name:
        return "ULTRA"
    if "VIP" in name:
        return "EPIC"
...
def shop_text():
    ...
    lines = [
        ...
        "╚══════════════════════╝",
        "",
        "🪙 Spend coins • Unlock rewards • Build your collection",
        "💡 Collectibles are virtual group items and have no real-world cash value.",
        "",
    ]
    rarity_icons = {"STANDARD":"▫️", "RARE":"🔹", "EPIC":"🟣", "ULTRA":"🔷", "MYTHIC":"🟪", "LEGENDARY":"🟡"}
    for idx, (item_id, name, desc, price, reward_type, reward_value) in enumerate(items, 1):
        rarity = rarity_for_item(name, reward_type)
        icon = rarity_icons.get(rarity, "🔹")
        lines.append(f"{idx}. {name}  •  🪙 {price:,}")
        lines.append(f"   {icon} {rarity}  |  {desc}")
        lines.append("")
    lines.append("━━━━━━━━━━━━━━━━━━━━━━")
    lines.append("👇 Tap any reward below to claim it")
    lines.append("🎒 /inventory  •  🪙 /coins  •  🏆 /coinrank")
    return "\n".join(lines)
...
def shop_keyboard():
    ...
    rows.append([
        ...
        InlineKeyboardButton("💳 WALLET", callback_data="shop_wallet"),
    ])
    rows.append([InlineKeyboardButton("🔄 REFRESH SHOP", callback_data="shop")])
...
def add_inventory_item(chat_id, user_id, item_id, item_name, rarity):
    now = datetime.utcnow().isoformat()
    with db_connect() as conn:
        conn.execute(
            "INSERT INTO inventories(chat_id,user_id,item_id,item_name,rarity,quantity,acquired_at) VALUES(?,?,?,?,?,1,?) "
            "ON CONFLICT(chat_id,user_id,item_id) DO UPDATE SET quantity=inventories.quantity+1, acquired_at=excluded.acquired_at",
            (chat_id, user_id, item_id, item_name, rarity, now)
        )
        row = conn.execute(
            "SELECT quantity FROM inventories WHERE chat_id=? AND user_id=? AND item_id=?",
...
def get_inventory(chat_id, user_id):
    with db_connect() as conn:
        return conn.execute(
            "SELECT item_id,item_name,rarity,quantity FROM inventories WHERE chat_id=? AND user_id=? ORDER BY quantity DESC,item_id",
            (chat_id, user_id)
        ).fetchall()
...
async def inventory_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user = update.effective_user
    if not chat or chat.type not in ("group", "supergroup"):
        await update.effective_message.reply_text("⚠️ /inventory group mein use karo.")
        return
    items = get_inventory(chat.id, user.id)
    if not items:
        await update.effective_message.reply_text(
            "🎒 𝐈𝐍𝐕𝐄𝐍𝐓𝐎𝐑𝐘\n\n"
            "Abhi tumhari collection empty hai. 🥲\n"
            "🛒 /shop se rare items collect karo!"
        )
        return
    icons = {"STANDARD":"▫️", "RARE":"🔹", "EPIC":"🟣", "ULTRA":"🔷", "MYTHIC":"🟪", "LEGENDARY":"🟡"}
    lines = ["╔══════════════════════╗", "   🎒 𝐌𝐘 𝐂𝐎𝐋𝐋𝐄𝐂𝐓𝐈𝐎𝐍", "╚══════════════════════╝", ""]
    total = 0
    for _, name, rarity, qty in items:
        total += qty
        lines.append(f"{icons.get(rarity, '🔹')} {name} × {qty}  [{rarity}]")
    lines += ["", f"📦 Total collectibles: {total}", "🛒 /shop to collect more rare items."]
    await update.effective_message.reply_text("\n".join(lines), reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🛒 OPEN SHOP", callback_data="shop")]]))


async def coinrank_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat = update.effective_chat
    if not chat or chat.type not in ("group", "supergroup"):
        await update.effective_message.reply_text("⚠️ /coinrank group mein use karo.")
        return
    with db_connect() as conn:
        rows = conn.execute(
            "SELECT user_id,balance FROM coins WHERE chat_id=? ORDER BY balance DESC LIMIT 10",
            (chat.id,)
        ).fetchall()
    if not rows:
        await update.effective_message.reply_text("🪙 Abhi coin leaderboard empty hai.")
        return
    lines = ["🏆 𝐂𝐎𝐈𝐍 𝐋𝐄𝐀𝐃𝐄𝐑𝐁𝐎𝐀𝐑𝐃", ""]
    medals = ["🥇", "🥈", "🥉"]
    for idx, (uid, balance) in enumerate(rows, 1):
        try:
...
async def shop_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat = update.effective_chat
    if not chat or chat.type not in ("group", "supergroup"):
        await update.effective_message.reply_text("⚠️ /shop group mein use karo.")
        return
    await update.effective_message.reply_text(shop_text(), reply_markup=shop_keyboard())
...
async def coins_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user = update.effective_user
    if not chat or chat.type not in ("group", "supergroup"):
        await update.effective_message.reply_text("⚠️ /coins group mein use karo.")
        return
...
async def dailycoins_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user = update.effective_user
    if not chat or chat.type not in ("group", "supergroup"):
        await update.effective_message.reply_text("⚠️ /dailycoins group mein use karo.")
        return
...
async def gift_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not chat or chat.type not in ("group", "supergroup"):
        await update.effective_message.reply_text("⚠️ /gift group mein use karo.")
        return
    target_msg = update.effective_message.reply_to_message
    target = target_msg.from_user if target_msg and target_msg.from_user else None
    if not target or target.is_bot:
        await update.effective_message.reply_text("🎁 Jis member ko gift dena hai, uske message ko reply karke use karo: /gift 100")
        return
    if target.id == sender.id:
        await update.effective_message.reply_text("😄 Khud ko gift nahi kar sakte.")
        return
    if len(context.args) != 1 or not context.args[0].isdigit():
        await update.effective_message.reply_text("🎁 Format: reply to member + /gift 100")
        return
    amount = int(context.args[0])
    if amount <= 0:
        await update.effective_message.reply_text("❌ Gift amount 0 se zyada hona chahiye.")
        return
    sender_balance = get_coins(chat.id, sender.id)
    if sender_balance < amount:
        await update.effective_message.reply_text(f"❌ Tumhare paas sirf {sender_balance} coins hain.")
        return
...
async def runall_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not owner_only(update):
        await update.effective_message.reply_text("❌ Sirf owner /runall use kar sakta hai.")
        return
    chat = update.effective_chat
    user = update.effective_user
    if not chat or chat.type not in ("group", "supergroup"):
        await update.effective_message.reply_text("⚠️ /runall group mein use karo.")
        return
...
async def profile_command(update, context):
    ...
    user = update.effective_user
    if not chat or chat.type not in ("group", "supergroup"):
        await update.effective_message.reply_text("⚠️ /profile sirf group mein use karo.")
        return
...
async def settitle_command(update, context):
    ...
    if not chat or chat.type not in ("group", "supergroup") or not target or target.is_bot or not title:
        await update.effective_message.reply_text("⚠️ Member ke message ko reply karke /settitle CODER use karo.")
        return
    if len(title) > 32:
        await update.effective_message.reply_text("❌ Title maximum 32 characters ka ho sakta hai.")
        return
    set_custom_title(chat.id, target.id, title)
    await update.effective_message.reply_text(f"✅ {target.full_name} ka custom title set: {title}")
...
async def cleartitle_command(update, context):
    ...
    if not chat or chat.type not in ("group", "supergroup") or not target:
        await update.effective_message.reply_text("⚠️ Member ke message ko reply karke /cleartitle use karo.")
        return
    clear_custom_title(chat.id, target.id)
    await update.effective_message.reply_text(f"✅ Custom title cleared. {target.full_name} ab default role title use karega.")


async def redeem_command(update, context):
    if not context.args:
        await update.effective_message.reply_text("🎟️ Use: /redeem CODE\nExample: /redeem SV-XXXXXXXXXX")
        return
    code = context.args[0].strip().upper()
    with db_connect() as conn:
        row = conn.execute("SELECT amount,used_by FROM redeem_codes WHERE code=?", (code,)).fetchone()
        if not row:
            await update.effective_message.reply_text("❌ Invalid redeem code.")
            return
        amount, used_by = row
        if used_by is not None:
            await update.effective_message.reply_text("⚠️ Ye redeem code already used hai.")
            return
    ...
    await update.effective_message.reply_text(f"🎉 𝐂𝐎𝐃𝐄 𝐑𝐄𝐃𝐄𝐄𝐌𝐄𝐃!\n\n🪙 +{amount} coins added.\n💰 Balance: {new_balance} coins")
...
async def analytics_command(update, context):
    ...
    user = update.effective_user
    if not chat or chat.type not in ("group", "supergroup"):
        await update.effective_message.reply_text("⚠️ /analytics sirf group mein use karo.")
        return
    try:
        member = await context.bot.get_chat_member(chat.id, user.id)
        if member.status not in ("creator", "administrator") and user.id != OWNER_ID:
            await update.effective_message.reply_text("❌ Sirf group admins /analytics use kar sakte hain.")
            return
    except Exception:
        if user.id != OWNER_ID:
            await update.effective_message.reply_text("❌ Admin permission verify nahi ho saki.")
    ...
    await update.effective_message.reply_text(f"📊 𝐆𝐑𝐎𝐔𝐏 𝐀𝐍𝐀𝐋𝐘𝐓𝐈𝐂𝐒\n\n👥 Known members: {members}\n🔥 Active (7 days): {active}\n💬 Total messages: {total_messages}\n🪙 Total coins: {coins}\n\n🏆 Top active members:\n{top_text}")
...
async def autoclean_command(update, context):
    ...
    chat = update.effective_chat
    if not chat or chat.type not in ("group", "supergroup"):
        await update.effective_message.reply_text("⚠️ /autoclean group mein use karo.")
        return
    ...
    await update.effective_message.reply_text(f"🧹 Auto-Clean: {'ON 🟢' if enabled else 'OFF 🔴'}\n\nBot commands will be cleaned after the configured delay.")
...
async def rank_command(update, context):
    ...
    user = update.effective_user
    if not chat or chat.type not in ("group", "supergroup"):
        await update.effective_message.reply_text("⚠️ /rank sirf group mein use karo.")
        return
...
async def top_command(update, context):
    chat = update.effective_chat
    if not chat or chat.type not in ("group", "supergroup"):
        await update.effective_message.reply_text("⚠️ /top sirf group mein use karo.")
        return
    rows = get_group_top(chat.id, 10)
    if not rows:
        await update.effective_message.reply_text("📊 Abhi leaderboard empty hai. Group mein messages bhejo aur XP earn karo! 🚀")
        return
...
async def register_group(update, context):
    ...
    user = update.effective_user
    if not chat or chat.type not in ("group", "supergroup"):
        await update.effective_message.reply_text("⚠️ Ye command apne group mein use karo.")
        return
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
    # Channel posts do not have an effective_user. /start in a channel
    # therefore becomes a channel-safe status/menu response.
    if not user:
        ...
        if msg:
            await msg.reply_text(
                ...
                "🤖 Bot is connected to this channel.\n\n"
                "ℹ️ Member-based features like XP, Coins, VIP/Elite and Profile "
                "work in private chat/groups because Telegram channel posts do not "
                "provide the posting member's user identity.",
...
async def button_handler(
    ...
    if query.data.startswith("giveaway_join:"):
        ...
        items = get_inventory(query.message.chat_id, user.id)
        if not items:
            await query.answer("🎒 Your collection is empty.", show_alert=True)
            return
        text = ["🎒 𝐌𝐘 𝐂𝐎𝐋𝐋𝐄𝐂𝐓𝐈𝐎𝐍", ""]
        for _, name, rarity, qty in items:
            text.append(f"{name} × {qty} • {rarity}")
        await query.edit_message_text("\n".join(text), reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🛒 BACK TO SHOP", callback_data="shop")]]))
        ...
        elif reward_type.startswith("collectible:"):
            rarity = reward_type.split(":", 1)[1]
            qty = add_inventory_item(query.message.chat_id, user.id, item_id, name, rarity)
            reward_msg = f"🎒 Added to your collection • {rarity}\n📦 You now own: ×{qty}"
...
/roast [name] — Get a random funny roast 😂
...
💡 𝐓𝐈𝐏: Commands ko group mein slash `/` ke saath use karo.
...
async def user_id(update, context):
    if not update.effective_user:
        await update.effective_message.reply_text(
            "ℹ️ Channel posts don't expose the posting member's Telegram user ID. "
            "Use /id in the bot's private chat or a group."
        )
        return
    await update.effective_message.reply_text(
...
async def userinfo_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not target:
        await update.effective_message.reply_text("ℹ️ User info available nahi hai.")
        return
    username = f"@{target.username}" if target.username else "Not set"
    role = "MEMBER"
    if chat and chat.type in ("group", "supergroup"):
        try:
            member = await context.bot.get_chat_member(chat.id, target.id)
            role = str(member.status).upper()
        except Exception:
            pass
    lines = [
        "╔══════════════════════╗",
...
async def chatid_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not chat:
        return
    await update.effective_message.reply_text(
        f"💬 𝐂𝐇𝐀𝐓 𝐈𝐍𝐅𝐎\n\n🏷️ Title: {chat.title or 'Private Chat'}\n🆔 Chat ID: {chat.id}\n📌 Type: {chat.type}"
    )
...
async def movie_command(update, context):
    ...
    if data and data.get("results"):
        ...
        await update.effective_message.reply_text(text, reply_markup=movie_search_keyboard(query))
    else:
        await update.effective_message.reply_text(
...
async def series_command(update, context):
    ...
    if data and data.get("results"):
        ...
        await update.effective_message.reply_text(text, reply_markup=series_search_keyboard(query))
    else:
        await update.effective_message.reply_text(
            f"🔎 No live TMDB result found for: {query}\n\n"
            "Try a different spelling or use the search buttons below.",
...
async def referral_command(update, context):
    ...
    try:
        ...
        await update.effective_message.reply_text(f"❌ Referral error: {e}")
...
async def giveaway_command(update, context):
    ...
    user = update.effective_user
    if not chat or chat.type not in ("group", "supergroup"):
        await update.effective_message.reply_text("⚠️ /giveaway sirf group mein use karo.")
        return
    try:
        member = await context.bot.get_chat_member(chat.id, user.id)
        if member.status not in ("creator", "administrator") and user.id != OWNER_ID:
            await update.effective_message.reply_text("❌ Sirf group admins /giveaway chala sakte hain.")
            return
    except Exception:
        if user.id != OWNER_ID:
            return
    if len(context.args) < 3:
        await update.effective_message.reply_text("🎁 Use: /giveaway MINUTES WINNERS PRIZE\nExample: /giveaway 60 2 1000 Coins")
        return
    try:
        minutes = int(context.args[0]); winners = int(context.args[1])
    except ValueError:
        await update.effective_message.reply_text("❌ Minutes aur winners number hone chahiye.")
        return
    if minutes < 1 or minutes > 10080 or winners < 1 or winners > 20:
        await update.effective_message.reply_text("❌ Minutes 1-10080 aur winners 1-20 rakho.")
        return
    ...
    msg = await update.effective_message.reply_text(f"🎉 𝐆𝐈𝐕𝐄𝐀𝐖𝐀𝐘 𝐋𝐈𝐕𝐄!\n\n🎁 Prize: {prize}\n🏆 Winners: {winners}\n⏳ Ends in: {minutes} minutes\n\n👇 Join below!", reply_markup=kb)
    with db_connect() as conn:
...
async def tag_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not chat or chat.type not in ("group", "supergroup"):
        await update.effective_message.reply_text("⚠️ /tag command sirf group mein use karo.")
        return
    ...
    try:
        member = await context.bot.get_chat_member(chat.id, user.id)
        if member.status not in ("creator", "administrator") and user.id != OWNER_ID:
            await update.effective_message.reply_text("❌ Sirf group admins / owner /tag use kar sakte hain.")
            return
    except Exception:
        if user.id != OWNER_ID:
            await update.effective_message.reply_text("❌ Admin permission verify nahi ho saki.")
    ...
    if chat.id in tag_running:
        await update.effective_message.reply_text("⏳ Tagging already chal rahi hai. Please wait...")
        return
...
async def welcome_toggle_command(update, context):
    ...
    if update.effective_chat.type not in ("group", "supergroup"):
        await update.effective_message.reply_text("⚠️ Is command ko group mein use karo.")
        return
    key = f"welcome:{update.effective_chat.id}"
    new = "0" if get_setting(key, "1") == "1" else "1"
    set_setting(key, new)
    await update.effective_message.reply_text(f"👋 Welcome system: {'ON 🟢' if new == '1' else 'OFF 🔴'}")
...
async def promo_on(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    AUTO_PROMO_ENABLED = True
    set_setting("promo_enabled", "1")
    await update.effective_message.reply_text("✅ Auto-promo ON hai.")
...
async def promo_off(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.effective_message.reply_text("🛑 Auto-promo OFF kar diya gaya hai.")
...
async def channel_post_support(update, context):
    """Channel-safe processing for non-command posts.

    Telegram channel posts have no effective_user, so member-specific
    features cannot safely be attributed to a subscriber. Commands are
    handled by CommandHandler; this handler only provides a small channel
    status response for ordinary bot-directed text and deliberately does
    not create fake member accounts.
    """
    chat = update.effective_chat
    msg = update.effective_message
    if not chat or chat.type != "channel" or not msg:
        return
    text = (msg.text or "").strip().lower()
    if text in ("bot", "bot status", "status"):
        await msg.reply_text(
            "🤖 𝐁𝐎𝐓 𝐒𝐓𝐀𝐓𝐔𝐒\n\n"
            "✅ Connected to this channel\n"
            "📢 Channel support: ACTIVE\n"
            "⚠️ Member-based commands work in private chat/groups."
        )
...
ROASTS = [
    '{name}, tera attitude 4K hai, performance abhi bhi 144p 😂',
    '{name}, tu roast hone nahi aaya, khud content banke aaya hai 🤣',
    '{name}, Google bhi tera dimaag search karke No Results Found bol de 😂',
    '{name}, confidence full hai, talent loading mein hai… ⏳😂',
    '{name}, teri entry se zyada tera network lag karta hai 📶🤣',
    '{name}, tu itna confused hai ki GPS bhi tujhe direction dene se pehle sochta hai 😂',
    '{name}, tera logic dekh ke calculator ne bhi resignation de diya 🧮😭',
    '{name}, tu online aata hai aur Wi-Fi khud slow ho jata hai 📶😂',
    '{name}, tera comeback itna late hota hai ki conversation hi expire ho jaati hai 🤣',
    '{name}, tera brain abhi bhi Please Wait… pe stuck hai ⏳😂',
    '{name}, tu smart banne ki koshish karta hai, par system requirements match nahi karti 😭😂',
    '{name}, tera plan sunke Plan Failed Successfully yaad aa gaya 🤣',
    '{name}, teri speed dekh ke loading screen bhi impatient ho jaye 😂',
    '{name}, tu advice aise deta hai jaise khudki life tutorial mode mein ho 🤣',
    '{name}, tera confidence premium hai, result free trial 😭😂',
    '{name}, tu itna slow hai ki snail bhi tujhe overtake karke horn maare 🐌🤣',
    '{name}, tera dimaag airplane mode pe hai aur tu signal dhoond raha hai 😂',
    '{name}, teri timing dekh ke alarm clock bhi jealous ho gaya ⏰🤣',
    '{name}, tu problem solve karne gaya tha, problem ka part ban gaya 😂',
    '{name}, tera logic update maang raha hai… version kaafi purana hai 🤣',
    '{name}, tu screenshot jaisa hai—dekho sab rahe hain, samajh koi nahi raha 😂',
    '{name}, tera confidence dekh ke lagta hai battery 100%, par charger andar se disconnected hai 🔋🤣',
    '{name}, tu itna unpredictable hai ki random button bhi predictable lagne lage 😂',
    '{name}, tera dimaag tabhi fast chalta hai jab topic tera ho 🤣',
    '{name}, tu legend banne nikla tha, loading screen pe hi legendary ho gaya 😂🔥',
    '{name}, tera swag online hai, par common sense offline hai 😂',
    '{name}, tu shortcut dhoondte-dhoondte long route ka champion ban gaya 🤣',
    '{name}, tera focus Wi-Fi signal jaisa hai—kabhi strong, kabhi gayab 📶😂',
    '{name}, tu itna unique hai ki autocorrect bhi tujhe correct karna chhod de 😭🤣',
    '{name}, teri planning dekh ke calendar bhi confused hai 📅😂',
    '{name}, tu jab serious hota hai tab bhi comedy automatic chalti hai 🤣',
    '{name}, tera dimaag 1% battery pe bhi notification de raha hai 😂🔋',
    '{name}, tu answer se pehle explanation ka trailer chala deta hai 🎬🤣',
    '{name}, teri speed dekhkar turtle ne bhi bola: bhai thoda fast 😂🐢',
    '{name}, tu itna late reply karta hai ki message khud retirement le leta hai 🤣',
    '{name}, tera style premium lagta hai, par update pending hai 😂',
    '{name}, tu logic ko follow nahi karta, logic tujhe follow karte-karte thak gaya 😭🤣',
    '{name}, teri memory ka backup shayad trial version hai 😂',
    '{name}, tu ek aisa notification hai jise sab dekhte hain, ignore bhi karte hain 🤣',
    '{name}, tera confidence dekh ke lagta hai result abhi download ho raha hai ⏳😂',
    '{name}, tu multitasking karta hai: ek saath sab kuch confuse kar deta hai 🤣',
    '{name}, tera idea sunke brain ne safe mode on kar liya 😂',
    '{name}, tu instructions padhne se pehle hi experiment start kar deta hai 😭🤣',
    '{name}, teri typing speed fast hai, par meaning abhi processing mein hai 😂',
    '{name}, tu debate jeetne nahi, topic badalne mein expert hai 🤣',
    '{name}, tera calculator bhi tere maths pe emotional ho jata hai 🧮😂',
    '{name}, tu itna optimistic hai ki wrong answer mein bhi confidence dhoond leta hai 🤣',
    '{name}, tera comeback draft mein hi legendary reh gaya 😂',
    '{name}, tu jab bolta hai ‘easy hai’, tab sabko tension hone lagti hai 😭🤣',
    '{name}, tera brain aur internet dono kabhi-kabhi reconnect maangte hain 📶😂',
    '{name}, tu map ke bina bhi lost hone ka talent rakhta hai 🤣',
    '{name}, teri strategy dekh ke opponent bhi confused ho jata hai 😂',
    '{name}, tu simple question ko bhi season finale bana deta hai 🎬🤣',
    '{name}, tera common sense vacation pe gaya hua lagta hai 😂',
    '{name}, tu update hone ke baad bhi old version jaisa feel deta hai 🤣',
    '{name}, tera confidence unlimited hai, par results data pack jaise limited 😂',
    '{name}, tu joke samajhne mein itna time leta hai ki punchline ko bhi explain karna padta hai 🤣',
    '{name}, teri speed ko dekhkar loading bar ne motivation lose kar diya 😂',
    '{name}, tu problem ko dekhkar solution nahi, naya problem bana deta hai 🤣',
    '{name}, tera brain tabs bahut kholta hai, par koi load nahi hota 😂',
    '{name}, tu online hoke bhi kabhi-kabhi offline lagta hai 🤣',
    '{name}, teri logic ki battery low hai, charger bhi compatible nahi 😂',
    '{name}, tu confidence ka full pack hai, bas skill ka recharge karna baaki hai 🤣',
    '{name}, tera plan A fail hua to plan B bhi vacation pe chala gaya 😂',
    '{name}, tu itna creative hai ki simple kaam ko complicated bana deta hai 🤣',
    '{name}, teri explanation sunke question aur zyada mysterious ho gaya 😂',
    '{name}, tu keyboard pe fast hai, decisions mein loading chalti hai 🤣',
    '{name}, tera brain kabhi-kabhi ‘try again later’ dikha deta hai ⏳😂',
    '{name}, tu perfect timing ka example hai—mostly wrong timing 🤣',
    '{name}, tera logic dekhkar calculator ne dark mode on kar liya 😂',
    '{name}, tu group ka notification hai—attention guaranteed, reason questionable 🤣',
    '{name}, teri strategy itni secret hai ki tujhe khud nahi pata 😂',
    '{name}, tu jab shortcut leta hai, destination bhi surprise ho jata hai 🤣',
    '{name}, tera confidence dekhkar lagta hai tutorial skip karke final boss pe aa gaya 😂',
    '{name}, tu har situation mein plot twist add kar deta hai 🤣',
    '{name}, tera brain ka loading icon permanent resident lagta hai 😂',
    '{name}, tu advice dene mein fast hai, follow karne mein slow 🤣',
    '{name}, teri memory ka refresh button missing hai 😂',
    '{name}, tu ek walking ‘Are you sure?’ popup hai 🤣',
    '{name}, tera logic kabhi-kabhi beta version mein chalta hai 😂',
    '{name}, tu confidence se wrong answer bolta hai, respect for consistency 🤣',
    '{name}, tera focus 5G hai, par attention 2G 😂',
    '{name}, tu ek hi kaam ko teen baar explain karwa sakta hai 🤣',
    '{name}, teri planning mein surprise zyada, planning kam hai 😂',
    '{name}, tu jab solution deta hai, problem bhi sochti hai ki ye kya ho raha hai 🤣',
    '{name}, tera brain notifications ko priority nahi deta 😂',
    '{name}, tu deadline ko suggestion samajhta hai 🤣',
    '{name}, tera confidence dekhkar lagta hai homework ne tujhe hi submit kar diya 😂',
    '{name}, tu har conversation mein apna bonus level unlock kar leta hai 🤣',
    '{name}, tera logic kabhi straight line mein travel nahi karta 😂',
    '{name}, tu simple button ko bhi advanced feature bana deta hai 🤣',
    '{name}, tera reaction 100%, reason 0% 😂',
    '{name}, tu question ka answer deta hai aur question hi confuse ho jata hai 🤣',
    '{name}, teri decision-making mein random button ka influence zyada hai 😂',
    '{name}, tu ‘bas 2 minute’ bolkar time ko challenge karta hai 🤣',
    '{name}, tera brain cache clear karne ka time aa gaya 😂',
    '{name}, tu har baar confidence ke saath unexpected result deliver karta hai 🤣',
    '{name}, teri entry se silence bhi awkward ho jata hai 😂',
    '{name}, tu itna chill hai ki urgency bhi tujhe reminder bhejti hai 🤣',
    '{name}, tera dimaag multitask karta hai: confuse bhi, entertain bhi 😂',
    '{name}, tu calculator nahi, calculation ka plot twist hai 🤣',
    '{name}, tera style ready hai, bas logic ka update pending hai 😂',
    '{name}, tu har plan mein ek hidden ‘oops’ feature rakhta hai 🤣',
    '{name}, teri timing ko dekhkar clock bhi second-guess kare ⏰😂',
    '{name}, tu jab shortcut explain karta hai, tutorial ban jata hai 🤣',
    '{name}, tera confidence HD hai, execution buffering mein 😂',
    '{name}, tu group chat ka unexpected notification hai 🤣',
    '{name}, tera brain restart maangta hai, par tu continue pe click kar deta hai 😂',
    '{name}, tu ek hi line mein itne plot twists daal deta hai ki movie bhi jealous ho jaye 🤣',
    '{name}, teri speed aur loading screen ki dosti purani lagti hai 😂',
    '{name}, tu har baar prove karta hai ki unexpected bhi predictable ho sakta hai 🤣',
    '{name}, tera common sense kabhi-kabhi guest appearance karta hai 😂',
    '{name}, tu confidence ka notification hai—baar-baar pop up hota hai 🤣',
    '{name}, tera logic airplane mode se bhi zyada silent hai 😂',
    '{name}, tu problem ko solve nahi karta, uska sequel bana deta hai 🤣',
    '{name}, teri planning ka best part hai—ending koi predict nahi kar sakta 😂',
    '{name}, tu jab ‘trust me’ bolta hai, tab backup plan ready rakhna chahiye 🤣',
    '{name}, tera dimaag kabhi-kabhi demo mode mein chal raha hota hai 😂',
    '{name}, tu har simple task ko achievement unlock jaisa treat karta hai 🏆🤣',
    '{name}, tera confidence dekhkar lagta hai result ka screenshot pehle hi le liya 😂',
    '{name}, tu answer se zyada reaction mein marks leta hai 🤣',
    '{name}, tera brain ka search bar busy hai, result abhi pending hai 😂',
    '{name}, tu logic ko confuse nahi karta, logic khud confused ho jata hai 🤣',
    '{name}, tera plan itna flexible hai ki plan ko khud nahi pata kya karna hai 😂',
    '{name}, tu har conversation mein ek bonus comedy scene de deta hai 🤣',
    '{name}, tera focus kabhi-kabhi auto-update pe chala jata hai 😂',
    '{name}, tu confidence se bolta hai aur reality quietly ‘hmm’ karti hai 🤣',
    '{name}, tera dimaag loading se zyada buffering ka fan hai 😂',
    '{name}, tu har baar naya tareeka dhoond leta hai same confusion ka 🤣',
    '{name}, tera logic ka password shayad khud tujhe nahi pata 😂',
    '{name}, tu group ka unofficial entertainment department hai 🤣🔥',
]


async def roast_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Send one or multiple random light/funny roasts."""
    target = "Bhai"
    count = 1

    args = list(context.args or [])

    # Supports: /roast @username 15
    if args:
        if args[-1].isdigit():
            count = int(args.pop())
        if args:
    ...
    # Supports: replying to a member with /roast or /roast 15
    if not args and update.effective_message and update.effective_message.reply_to_message:
        replied = update.effective_message.reply_to_message.from_user
        if replied:
            target = replied.full_name or replied.username or "Bhai"

    # Keep one command from flooding a chat; max 100 roasts per request.
    count = max(1, min(count, 100))
    selected = random.sample(ROASTS, min(count, len(ROASTS)))

    if count == 1:
        output = f"🔥 𝐑𝐎𝐀𝐒𝐓 𝐓𝐈𝐌𝐄 🔥\n\n{selected[0].format(name=target)}"
        await update.effective_message.reply_text(output)
        return

    # Send every roast as a separate message instead of one combined message.
    for i, roast in enumerate(selected, 1):
        await update.effective_message.reply_text(
            f"🔥 𝐑𝐎𝐀𝐒𝐓 #{i} 🔥\n\n{roast.format(name=target)}"
        )
        # Small pause helps avoid Telegram flood limits when sending multiple messages.
        if i < len(selected):
...
async def run_bot():
    ...
    app.add_handler(CommandHandler("coinrank", coinrank_command))
