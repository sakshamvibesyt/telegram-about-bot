...
MINIAPP_SESSION_DAYS = int(os.environ.get("MINIAPP_SESSION_DAYS", "7"))
...
async def miniapp_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not MINIAPP_URL:
        await update.message.reply_text(
            "⚠️ Mini App URL configured nahi hai. Render environment me MINIAPP_URL set karo."
        )
        return
    keyboard = [[InlineKeyboardButton("🚀 OPEN MINI APP", web_app=WebAppInfo(url=MINIAPP_URL))]]
    await update.message.reply_text(
        "✨ 𝐒𝐀𝐊𝐒𝐇𝐀𝐌 𝐌𝐈𝐍𝐈 𝐀𝐏𝐏\n\nProfile, wallet, XP, rewards aur membership ek hi jagah.\n\n👇 Open karne ke liye button dabao:",
        reply_markup=InlineKeyboardMarkup(keyboard),
    )

# ==================================
# FLASK SERVER + TELEGRAM MINI APP BACKEND
# ==================================
...
def init_miniapp_db():
    with miniapp_db_connect() as conn:
        ...
        conn.execute("CREATE INDEX IF NOT EXISTS idx_app_sessions_expires ON app_sessions(expires_at)")
        conn.execute("""
...
def _miniapp_hash(value):
    import hashlib
    return hashlib.sha256(value.encode("utf-8")).hexdigest()


def verify_telegram_init_data(init_data):
    """Verify Telegram WebApp initData on the server. Never trust initDataUnsafe."""
    if not init_data or not BOT_TOKEN:
        return None, "Telegram authentication data is missing. Open this app from Telegram."
    ...
    try:
        ...
        if not auth_date or abs(int(time.time()) - auth_date) > 86400:
            return None, "Telegram authentication data has expired. Please reopen the Mini App."
...
def _miniapp_init_user():
    return verify_telegram_init_data(request.headers.get("X-Telegram-Init-Data", ""))


def _miniapp_session_account(tg_user):
    token = request.cookies.get("miniapp_session", "")
    if not token:
        return None
    token_hash = _miniapp_hash(token)
    now = datetime.utcnow().isoformat()
    with miniapp_db_connect() as conn:
        row = conn.execute("""
            SELECT a.id, a.telegram_id, a.username, a.password_hash
            FROM app_sessions s
        ...
        if not row:
            ...
            return None
        if int(row["telegram_id"]) != int(tg_user["id"]):
            return None
        return row


def _miniapp_login_required():
    tg_user, error = _miniapp_init_user()
    if error:
        return None, None, (jsonify({"ok": False, "error": error}), 401)
    account = _miniapp_session_account(tg_user)
    if not account:
        return tg_user, None, (jsonify({"ok": False, "error": "Please login first."}), 401)
    return tg_user, account, None


def _miniapp_new_session(account_id):
    token = secrets.token_urlsafe(32)
    token_hash = _miniapp_hash(token)
    created = datetime.utcnow()
    expires = created + timedelta(days=MINIAPP_SESSION_DAYS)
    with miniapp_db_connect() as conn:
        conn.execute("DELETE FROM app_sessions WHERE account_id=?", (account_id,))
        conn.execute(
            "INSERT INTO app_sessions(token_hash,account_id,created_at,expires_at) VALUES(?,?,?,?)",
            (token_hash, account_id, created.isoformat(), expires.isoformat()),
        )
    return token, expires


def _miniapp_set_session(response, token, expires):
    response.set_cookie(
        "miniapp_session", token,
        max_age=MINIAPP_SESSION_DAYS * 86400,
        expires=expires,
        httponly=True,
        secure=True,
        samesite="Lax",
...
def _miniapp_bot_data(telegram_id):
    """Read-only aggregation from the existing bot database."""
    data = {
        "coins": 0,
        "xp": 0,
        "level": 1,
    ...
    try:
        with db_connect() as conn:
            row = conn.execute("SELECT COALESCE(SUM(balance),0) FROM coins WHERE user_id=?", (telegram_id,)).fetchone()
            data["coins"] = int(row[0] or 0)

            row = conn.execute("SELECT MAX(xp), MAX(level), COALESCE(SUM(message_count),0) FROM xp_levels WHERE user_id=?", (telegram_id,)).fetchone()
            if row:
                data["xp"] = int(row[0] or 0)
                data["level"] = int(row[1] or 1)
                data["messages"] = int(row[2] or 0)

            data["vip"] = bool(conn.execute("SELECT 1 FROM vip_users WHERE user_id=? LIMIT 1", (telegram_id,)).fetchone())
            data["elite"] = bool(conn.execute("SELECT 1 FROM elite_users WHERE user_id=? LIMIT 1", (telegram_id,)).fetchone())

            row = conn.execute("SELECT count FROM referral_counts WHERE user_id=?", (telegram_id,)).fetchone()
            data["referrals"] = int(row[0] or 0) if row else 0

            rows = conn.execute(
                "SELECT event, created_at FROM activity WHERE user_id=? ORDER BY rowid DESC LIMIT 10",
                (telegram_id,),
            ).fetchall()
            data["activity"] = [{"event": r[0], "created_at": r[1]} for r in rows]

            # Rank is based on the user's best XP group, when available.
            best = conn.execute("SELECT chat_id, xp FROM xp_levels WHERE user_id=? ORDER BY xp DESC LIMIT 1", (telegram_id,)).fetchone()
            if best:
                count = conn.execute(
                    "SELECT COUNT(*) + 1 FROM xp_levels WHERE chat_id=? AND xp>?",
                    (best[0], best[1]),
                ).fetchone()
                data["rank"] = int(count[0]) if count else None
    except Exception:
        # The Mini App must never break the existing bot if a legacy table is unavailable.
        pass
    return data
...
@web_app.route("/api/miniapp/me", methods=["GET"])
def miniapp_me():
    init_miniapp_db()
    tg_user, error = _miniapp_init_user()
    if error:
        return jsonify({"ok": False, "authenticated": False, "telegram_user": None, "error": error}), 401
    account = _miniapp_session_account(tg_user)
    return jsonify({
        "ok": True,
        "authenticated": bool(account),
        "telegram_user": tg_user,
        "account": {"username": account["username"]} if account else None,
    })
...
def miniapp_register():
    ...
    if not re.fullmatch(r"[A-Za-z0-9_]{3,32}", username):
        return jsonify({"ok": False, "error": "Username 3-32 characters ka ho: letters, numbers aur _ only."}), 400
    if len(password) < 8:
        return jsonify({"ok": False, "error": "Password minimum 8 characters ka hona chahiye."}), 400
    if password != confirm:
        return jsonify({"ok": False, "error": "Passwords match nahi karte."}), 400

    from werkzeug.security import generate_password_hash
    now = datetime.utcnow().isoformat()
    try:
        with miniapp_db_connect() as conn:
            if conn.execute("SELECT 1 FROM app_accounts WHERE telegram_id=?", (tg_user["id"],)).fetchone():
                return jsonify({"ok": False, "error": "Is Telegram account ka Mini App account already registered hai. Login karo."}), 409
            if conn.execute("SELECT 1 FROM app_accounts WHERE lower(username)=lower(?)", (username,)).fetchone():
                return jsonify({"ok": False, "error": "Ye username already taken hai."}), 409
            cur = conn.execute(
                "INSERT INTO app_accounts(telegram_id,username,password_hash,created_at,updated_at) VALUES(?,?,?,?,?)",
                (tg_user["id"], username, generate_password_hash(password), now, now),
            )
            account_id = cur.lastrowid
    except sqlite3.IntegrityError:
        return jsonify({"ok": False, "error": "Account already exists. Please login."}), 409

    token, expires = _miniapp_new_session(account_id)
    response = make_response(jsonify({"ok": True, "message": "Registration successful.", "username": username}))
    _miniapp_set_session(response, token, expires)
    return response
...
def miniapp_login():
    ...
    if not username or not password:
        return jsonify({"ok": False, "error": "Username aur password dono enter karo."}), 400

    from werkzeug.security import check_password_hash
    with miniapp_db_connect() as conn:
        account = conn.execute(
            "SELECT id,telegram_id,username,password_hash FROM app_accounts WHERE lower(username)=lower(?)",
            (username,),
        ).fetchone()
    if not account or int(account["telegram_id"]) != int(tg_user["id"]) or not check_password_hash(account["password_hash"], password):
        return jsonify({"ok": False, "error": "Username/password incorrect hai ya account kisi aur Telegram account se bound hai."}), 401

    token, expires = _miniapp_new_session(account["id"])
    response = make_response(jsonify({"ok": True, "message": "Login successful.", "username": account["username"]}))
    _miniapp_set_session(response, token, expires)
    return response
...
def miniapp_logout():
    ...
    if token:
        with miniapp_db_connect() as conn:
            conn.execute("DELETE FROM app_sessions WHERE token_hash=?", (_miniapp_hash(token),))
...
def miniapp_dashboard():
    tg_user, account, failure = _miniapp_login_required()
    if failure:
        return failure
    data = _miniapp_bot_data(tg_user["id"])
    return jsonify({
        "ok": True,
        "profile": {
...
@web_app.route("/api/miniapp/shop", methods=["GET"])
def miniapp_shop():
    tg_user, account, failure = _miniapp_login_required()
    if failure:
        return failure
    ...
    try:
        with db_connect() as conn:
            rows = conn.execute("SELECT item_id,name,description,price,reward_type,reward_value FROM coin_shop WHERE enabled=1 ORDER BY item_id").fetchall()
            items = [{"item_id": r[0], "name": r[1], "description": r[2], "price": r[3], "reward_type": r[4], "reward_value": r[5]} for r in rows]
    except Exception:
        pass
    return jsonify({"ok": True, "items": items, "coins": _miniapp_bot_data(tg_user["id"])["coins"]})
...
def miniapp_daily_reward():
    ...
    amount = random.randint(50, 150)
    with miniapp_db_connect() as conn:
        row = conn.execute("SELECT last_claim,total_claims FROM app_daily_rewards WHERE account_id=?", (account["id"],)).fetchone()
        if row and row["last_claim"] == today:
            return jsonify({"ok": False, "error": "Aaj ka reward already claim ho chuka hai."}), 409
        conn.execute(
            "INSERT INTO app_daily_rewards(account_id,last_claim,total_claims) VALUES(?,?,1) ON CONFLICT(account_id) DO UPDATE SET last_claim=excluded.last_claim,total_claims=app_daily_rewards.total_claims+1",
            (account["id"], today),
        )
    # Mini App reward is intentionally kept in the separate Mini App DB.
    return jsonify({"ok": True, "amount": amount, "message": f"🎁 Daily reward: +{amount} coins"})


@web_app.route("/api/miniapp/referral", methods=["GET"])
def miniapp_referral():
    tg_user, account, failure = _miniapp_login_required()
    if failure:
        return failure
    data = _miniapp_bot_data(tg_user["id"])
    return jsonify({"ok": True, "referrals": data["referrals"], "referral_link": f"https://t.me/{os.environ.get('BOT_USERNAME', 'your_bot')}?start=ref_{tg_user['id']}"})


@web_app.route("/api/miniapp/membership", methods=["GET"])
def miniapp_membership():
    tg_user, account, failure = _miniapp_login_required()
    if failure:
        return failure
    data = _miniapp_bot_data(tg_user["id"])
    return jsonify({"ok": True, "vip": data["vip"], "elite": data["elite"]})


@web_app.route("/api/miniapp/activity", methods=["GET"])
def miniapp_activity():
    tg_user, account, failure = _miniapp_login_required()
    if failure:
        return failure
    return jsonify({"ok": True, "activity": _miniapp_bot_data(tg_user["id"])["activity"]})
