...
/tagall [message] — Mention each known member in a separate message (admin)
...
async def tagall_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Send one separate clickable-mention message per known member.
    ...
    if not chat or chat.type not in ("group", "supergroup"):
        await update.message.reply_text("⚠️ /tagall command sirf group mein use karo.")
        return
    ...
    try:
        ...
        if member.status not in ("creator", "administrator") and user.id != OWNER_ID:
            await update.message.reply_text(
                "❌ Sirf group admins / owner /tagall use kar sakte hain."
            )
            return
    except Exception:
        if user.id != OWNER_ID:
    ...
    # Only members already detected/stored by the bot are available here.
    members = [(uid, name) for uid, name in group_members(chat.id) if uid and uid != user.id]

    if not members:
        await update.message.reply_text(
            "❌ Abhi koi known member nahi mila.\n\n"
            "Bot ko group ke messages/activities detect karne do, "
            "phir /tagall dobara use karo."
        )
        return

    # Everything after /tagall becomes the intro text.
    custom_text = " ".join(context.args).strip()
    intro = custom_text or "👋 Hey everyone!"
    ...
    try:
        ...
        await update.message.reply_text(
            f"📢 𝐓𝐀𝐆𝐀𝐋𝐋 𝐒𝐓𝐀𝐑𝐓𝐄𝐃\n\n"
            f"👥 Members: {total}\n"
            f"⚡ Har member ko alag message bheja jayega..."
        )

        # Use Telegram's HTML tg://user link instead of constructing a
        # MessageEntity/User object. This is more reliable for text mentions.
        from html import escape

        for uid, name in members:
            display_name = escape((name or "User")[:64], quote=False)
            mention = f'<a href="tg://user?id={int(uid)}">{display_name}</a>'
            message_text = f"{escape(intro, quote=False)}\n\n{mention}"
            ...
            try:
                ...
                failed += 1
                print(f"⚠️ /tagall send failed for user {uid}: {repr(e)}")
                # Continue to the next member instead of ending the whole run.
                continue

            # Separate message for every member.
            await asyncio.sleep(TAG_DELAY)
        ...
        print(f"⚠️ Separate /tagall error: {repr(e)}")
        try:
            await context.bot.send_message(
                chat_id=chat.id,
                text="⚠️ /tagall बीच में stop ho gayi. Logs check karo.",
            )
        except Exception:
            pass
    finally:
