...
async def tag_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Mention each known non-bot group member in a separate message.

    Usage: /tag [optional message]
    /tagall is mapped to this same handler.
    ...
    try:
        ...
        await update.message.reply_text(
            ...
            f"⚡ Har member ko separate message mein mention kiya ja raha hai..."
        )

        from telegram import MessageEntity, User

        # IMPORTANT: exactly ONE member per Telegram message.
        for uid, name in members:
            display_name = (name or "User")[:64]

            # Build a single text_mention entity for this one member.
            text = intro + "\n\n" + display_name
            offset = len((intro + "\n\n").encode("utf-16-le")) // 2
            length = len(display_name.encode("utf-16-le")) // 2
            ...
            try:
                ...
                print(f"⚠️ Tag error for {uid}: {e}")
                # Continue with the remaining members instead of stopping the
                # complete /tagall run because one member failed.
                continue
...
# ==================================
# AUTO-SAVE GROUP MEMBERS
# ==================================

async def track_chat_member_update(update, context):
    """Save users whenever they join/are added to a group.

    Bot API does not provide a full member-list endpoint, so this keeps the
    local member database updated from ChatMemberUpdated events.
    """
    ...
    if still_member:
        save_group_member(chat, user)
        print(f"👤 Saved group member: {user.id} / {user.full_name} in {chat.id}")
    elif status in ("left", "kicked"):
        # Don't keep users who have actually left/banned; if they join again
        # the next member update will automatically save them again.
        try:
            with db_connect() as conn:
                conn.execute(
            ...
            print(f"⚠️ Member cleanup error: {e}")
...
async def welcome_new_members(update, context):
    ...
    for member in update.message.new_chat_members:
        if member.is_bot:
            continue
        # Persist the member immediately so /tagall can use them even if they
        # have not sent a normal message yet.
        save_group_member(chat, member)
        save_user(member.id)
...
async def run_bot():
    ...
    # Telegram only sends chat_member updates when the bot is an admin and
    # the update type is explicitly enabled in polling.
    app.add_handler(
        ChatMemberHandler(
