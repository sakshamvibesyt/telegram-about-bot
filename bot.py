...
def lightweight_answer(q):
    ...
    answers={
        "hi":"👋 Hey! Main Saksham Vibes Bot hoon. /help se saare commands dekho.",
...
async def coinflip_command(update,context):
    chat=update.effective_chat; uid=update.effective_user.id
    result=random.choice(["HEADS 🪙","TAILS 🪙"]); game_count(chat.id,uid,False)
    await update.message.reply_text(f"🪙 𝐂𝐎𝐈𝐍 𝐅𝐋𝐈𝐏\n\n{result}")
...
async def battle_command(update,context):
    if not update.message.reply_to_message or not update.message.reply_to_message.from_user:
        await update.message.reply_text("⚔️ Kisi member ke message ko reply karke /battle use karo."); return
    a=update.effective_user; b=update.message.reply_to_message.from_user
    winner=random.choice([a,b]); game_count(update.effective_chat.id,a.id,winner.id==a.id)
    await update.message.reply_text(f"⚔️ 𝐁𝐀𝐓𝐓𝐋𝐄\n\n{a.full_name} VS {b.full_name}\n🏆 Winner: {winner.full_name} 🔥")
...
# ==================================
# FLASK SERVER / MINI APP BACKEND (SEPARATE AUTH DB)
# ==================================
web_app = Flask(__name__, static_folder="miniapp", static_url_path="/miniapp/static")

MINIAPP_URL = os.environ.get("MINIAPP_URL", "").strip()
MINIAPP_DB_FILE = os.environ.get("MINIAPP_DB_FILE", "miniapp_data.db")
MINIAPP_SESSION_DAYS = 30
MINIAPP_INITDATA_MAX_AGE = 86400


def miniapp_db_connect():
    conn = sqlite3.connect(MINIAPP_DB_FILE, timeout=30)
...
def init_miniapp_db():
    with miniapp_db_connect() as conn:
        conn.execute("""
        ...
        """)
        conn.execute("CREATE INDEX IF NOT EXISTS idx_app_sessions_account ON app_sessions(account_id)")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_app_sessions_expiry ON app_sessions(expires_at)")
        conn.execute("""
            CREATE TABLE IF NOT EXISTS app_daily_rewards (
...
def miniapp_norm_username(value):
...
def miniapp_json_error(message, status=400):
    return jsonify({"ok": False, "error": message}), status


def verify_telegram_init_data(init_data):
    """Validate Telegram Web App initData using the bot token.
    The client-provided initDataUnsafe/user fields are never trusted here.
    """
    if not init_data:
        return None, "Telegram authentication data missing."
    try:
        ...
        if auth_date <= 0 or time.time() - auth_date > MINIAPP_INITDATA_MAX_AGE or auth_date - time.time() > 60:
            return None, "Telegram authentication data has expired. Reopen the Mini App."
        data_check_string = "\n".join(f"{k}={parsed[k]}" for k in sorted(parsed))
        secret_key = hmac.new(b"WebAppData", BOT_TOKEN.encode(), hashlib.sha256).digest()
        calculated = hmac.new(secret_key, data_check_string.encode(), hashlib.sha256).hexdigest()
        if not hmac.compare_digest(calculated.lower(), received_hash.lower()):
            return None, "Telegram authentication could not be verified. Reopen the Mini App from Telegram."
        user_raw = parsed.get("user")
        if not user_raw:
            return None, "Telegram user information is missing."
        user = json.loads(user_raw)
        telegram_id = int(user["id"])
        return {"id": telegram_id, "user": user}, None
    except Exception:
        return None, "Invalid Telegram authentication data."


def miniapp_init_from_request():
    return request.headers.get("X-Telegram-Init-Data", "") or request.args.get("initData", "")


def miniapp_account_for_request(require_session=True):
    verified, error = verify_telegram_init_data(miniapp_init_from_request())
    if error:
        return None, None, error
    telegram_id = verified["id"]
    if not require_session:
        return None, verified, None
    token = request.headers.get("Authorization", "")
    if token.lower().startswith("bearer "):
        token = token[7:].strip()
    if not token:
        token = request.cookies.get("miniapp_session", "")
    if not token:
        return None, verified, "Please login first."
    token_hash = hashlib.sha256(token.encode()).hexdigest()
    now = miniapp_now().isoformat()
    with miniapp_db_connect() as conn:
        row = conn.execute("""
    ...
    if not row:
        return None, verified, "Session expired. Please login again."
    if int(row["telegram_id"]) != telegram_id:
        return None, verified, "This account is not bound to this Telegram account."
    return row, verified, None


def create_miniapp_session(account_id):
    raw = secrets.token_urlsafe(48)
    token_hash = hashlib.sha256(raw.encode()).hexdigest()
    now = miniapp_now()
    expires = now + timedelta(days=MINIAPP_SESSION_DAYS)
    with miniapp_db_connect() as conn:
        conn.execute("INSERT INTO app_sessions(token_hash,account_id,created_at,expires_at) VALUES(?,?,?,?)",
                     (token_hash, account_id, now.isoformat(), expires.isoformat()))
    return raw, expires


def miniapp_session_response(payload, token, expires):
    response = make_response(jsonify(payload))
    response.set_cookie(
        "miniapp_session", token,
        max_age=int((expires - miniapp_now()).total_seconds()),
        httponly=True, secure=True, samesite="Lax", path="/"
    )
    return response


def miniapp_public_account(row):
    return {
        "id": row["id"],
...
def miniapp_bot_snapshot(telegram_id, selected_chat_id=None):
    """Read existing bot_data.db only; Mini App auth DB stays independent."""
    result = {
        "coins": 0, "total_coins": 0, "xp": 0, "level": 1,
        "rank": None, "messages": 0, "status": "MEMBER", "title": "MEMBER",
        "chat_id": 0, "group_name": "No group data yet", "groups": [],
        "vip": False, "elite": False, "referrals": 0, "activity": []
    }
    try:
        with db_connect() as conn:
            group_rows = conn.execute("""
                SELECT DISTINCT x.chat_id, COALESCE(g.title, g.chat_id) AS title
                FROM xp_levels x LEFT JOIN group_chats g ON g.chat_id=x.chat_id
                WHERE x.user_id=? ORDER BY x.updated_at DESC
            """, (telegram_id,)).fetchall()
            groups = [{"chat_id": int(r[0]), "title": str(r[1])} for r in group_rows]
            result["groups"] = groups
            if selected_chat_id is not None:
                try: selected_chat_id = int(selected_chat_id)
                except Exception: selected_chat_id = None
            chat_id = selected_chat_id if selected_chat_id in {g["chat_id"] for g in groups} else (groups[0]["chat_id"] if groups else 0)
            result["chat_id"] = chat_id
            if chat_id:
                row = conn.execute("SELECT xp,level,message_count FROM xp_levels WHERE chat_id=? AND user_id=?", (chat_id, telegram_id)).fetchone()
                if row:
                    result["xp"], result["level"], result["messages"] = int(row[0]), int(row[1]), int(row[2])
                    result["rank"] = conn.execute("SELECT COUNT(*)+1 FROM xp_levels WHERE chat_id=? AND xp>?", (chat_id, row[0])).fetchone()[0]
                title = conn.execute("SELECT title FROM custom_titles WHERE chat_id=? AND user_id=?", (chat_id, telegram_id)).fetchone()
                if title: result["title"] = title[0]
                result["vip"] = bool(conn.execute("SELECT 1 FROM vip_users WHERE chat_id=? AND user_id=?", (chat_id, telegram_id)).fetchone())
                result["elite"] = bool(conn.execute("SELECT 1 FROM elite_users WHERE chat_id=? AND user_id=?", (chat_id, telegram_id)).fetchone())
                group = conn.execute("SELECT title FROM group_chats WHERE chat_id=?", (chat_id,)).fetchone()
                if group: result["group_name"] = group[0]
            total = conn.execute("SELECT COALESCE(SUM(balance),0) FROM coins WHERE user_id=?", (telegram_id,)).fetchone()[0]
            personal = conn.execute("SELECT COALESCE(balance,0) FROM coins WHERE chat_id=0 AND user_id=?", (telegram_id,)).fetchone()
            result["total_coins"] = int(total or 0)
            result["coins"] = int(personal[0] if personal else 0)
            if not chat_id:
                latest = conn.execute("SELECT xp,level,message_count,chat_id FROM xp_levels WHERE user_id=? ORDER BY updated_at DESC LIMIT 1", (telegram_id,)).fetchone()
                if latest:
                    result["xp"], result["level"], result["messages"], result["chat_id"] = map(int, latest)
            ref = conn.execute("SELECT count FROM referral_counts WHERE user_id=?", (telegram_id,)).fetchone()
            result["referrals"] = int(ref[0]) if ref else 0
            acts = conn.execute("SELECT event,created_at FROM activity WHERE user_id=? ORDER BY created_at DESC LIMIT 10", (telegram_id,)).fetchall()
            result["activity"] = [{"event": a[0], "created_at": a[1]} for a in acts]
    except Exception as exc:
        print(f"⚠️ Mini App bot-data read error: {exc}")
    return result
...
@web_app.route("/api/miniapp/status")
def miniapp_status():
    verified, error = verify_telegram_init_data(miniapp_init_from_request())
    if error: return miniapp_json_error(error, 401)
    with miniapp_db_connect() as conn:
        row = conn.execute("SELECT * FROM app_accounts WHERE telegram_id=?", (verified["id"],)).fetchone()
    return jsonify({"ok": True, "registered": bool(row), "telegram": verified["user"]})


@web_app.route("/api/miniapp/me")
def miniapp_me():
    account, verified, error = miniapp_account_for_request(True)
    if error:
        return jsonify({"ok": True, "authenticated": False, "registered": bool(verified and miniapp_account_exists(verified["id"]))})
    return jsonify({"ok": True, "authenticated": True, "registered": True, "account": miniapp_public_account(account), "telegram": verified["user"]})


def miniapp_account_exists(telegram_id):
    with miniapp_db_connect() as conn:
        return bool(conn.execute("SELECT 1 FROM app_accounts WHERE telegram_id=?", (telegram_id,)).fetchone())


@web_app.route("/api/miniapp/register", methods=["POST"])
def miniapp_register():
    verified, error = verify_telegram_init_data(miniapp_init_from_request())
    if error: return miniapp_json_error(error, 401)
    data = request.get_json(silent=True) or {}
    username = miniapp_norm_username(data.get("username"))
    password = str(data.get("password") or "")
    confirm = str(data.get("confirm_password") or "")
    if not username: return miniapp_json_error("Username 3-32 characters ka ho: letters, numbers aur underscore.")
    if len(password) < 8 or len(password) > 128: return miniapp_json_error("Password 8-128 characters ka hona chahiye.")
    if password != confirm: return miniapp_json_error("Passwords match nahi karte.")
    telegram_id = verified["id"]
    now = miniapp_now().isoformat()
    try:
        with miniapp_db_connect() as conn:
            if conn.execute("SELECT 1 FROM app_accounts WHERE telegram_id=?", (telegram_id,)).fetchone():
                return miniapp_json_error("Is Telegram account ka Mini App account already bana hua hai. Login karo.", 409)
            if conn.execute("SELECT 1 FROM app_accounts WHERE username=?", (username,)).fetchone():
                return miniapp_json_error("Ye username already taken hai.", 409)
            cur = conn.execute("INSERT INTO app_accounts(telegram_id,username,password_hash,created_at,updated_at) VALUES(?,?,?,?,?)",
                               (telegram_id, username, generate_password_hash(password), now, now))
            account_id = cur.lastrowid
        token, expires = create_miniapp_session(account_id)
        return miniapp_session_response({"ok": True, "session": token, "account": {"username": username}}, token, expires)
    except sqlite3.IntegrityError:
        return miniapp_json_error("Username ya Telegram account already registered hai.", 409)


@web_app.route("/api/miniapp/login", methods=["POST"])
def miniapp_login():
    verified, error = verify_telegram_init_data(miniapp_init_from_request())
    if error: return miniapp_json_error(error, 401)
    data = request.get_json(silent=True) or {}
    username = miniapp_norm_username(data.get("username"))
    password = str(data.get("password") or "")
    if not username or not password: return miniapp_json_error("Username aur password enter karo.")
    with miniapp_db_connect() as conn:
        account = conn.execute("SELECT * FROM app_accounts WHERE username=?", (username,)).fetchone()
    if not account or not check_password_hash(account["password_hash"], password):
        return miniapp_json_error("Username ya password galat hai.", 401)
    if int(account["telegram_id"]) != int(verified["id"]):
        return miniapp_json_error("Ye account kisi aur Telegram account se bound hai.", 403)
    token, expires = create_miniapp_session(account["id"])
    return miniapp_session_response({"ok": True, "session": token, "account": miniapp_public_account(account)}, token, expires)


@web_app.route("/api/miniapp/dashboard")
def miniapp_dashboard():
    account, verified, error = miniapp_account_for_request(True)
    if error: return miniapp_json_error(error, 401)
    data = miniapp_bot_snapshot(verified["id"], request.args.get("chat_id"))
    data["telegram"] = {"id": verified["id"], "name": verified["user"].get("first_name", "User"), "username": verified["user"].get("username", "")}
    data["account"] = miniapp_public_account(account)
    data["status"] = "OWNER" if verified["id"] == OWNER_ID else ("ELITE" if data["elite"] else ("VIP" if data["vip"] else "MEMBER"))
    return jsonify({"ok": True, "data": data})


@web_app.route("/api/miniapp/shop")
def miniapp_shop():
    account, verified, error = miniapp_account_for_request(True)
    if error: return miniapp_json_error(error, 401)
    try:
        with db_connect() as conn:
            rows = conn.execute("SELECT item_id,name,description,price,reward_type,reward_value FROM coin_shop WHERE enabled=1 ORDER BY price").fetchall()
        return jsonify({"ok": True, "items": [dict(r) for r in rows]})
    except Exception as exc:
        return miniapp_json_error(f"Shop load error: {exc}", 500)


@web_app.route("/api/miniapp/daily-reward", methods=["POST"])
def miniapp_daily_reward():
    account, verified, error = miniapp_account_for_request(True)
    if error: return miniapp_json_error(error, 401)
    today = miniapp_now().date().isoformat()
    with miniapp_db_connect() as conn:
        row = conn.execute("SELECT last_claim FROM app_daily_rewards WHERE telegram_id=?", (verified["id"],)).fetchone()
        if row and row[0] == today:
            return miniapp_json_error("Aaj ka daily reward already claim ho chuka hai.", 409)
        conn.execute("INSERT INTO app_daily_rewards(telegram_id,last_claim) VALUES(?,?) ON CONFLICT(telegram_id) DO UPDATE SET last_claim=excluded.last_claim", (verified["id"], today))
    new_balance = add_coins(0, verified["id"], 50)
    return jsonify({"ok": True, "reward": 50, "balance": new_balance})


@web_app.route("/api/miniapp/referral")
def miniapp_referral():
    account, verified, error = miniapp_account_for_request(True)
    if error: return miniapp_json_error(error, 401)
    try:
        me = get_referral_count(verified["id"])
        return jsonify({"ok": True, "referrals": me})
    except Exception:
        return jsonify({"ok": True, "referrals": 0})


@web_app.route("/api/miniapp/membership")
def miniapp_membership():
    account, verified, error = miniapp_account_for_request(True)
    if error: return miniapp_json_error(error, 401)
    data = miniapp_bot_snapshot(verified["id"])
    return jsonify({"ok": True, "vip": data["vip"], "elite": data["elite"], "status": data["status"]})


@web_app.route("/api/miniapp/activity")
def miniapp_activity():
    account, verified, error = miniapp_account_for_request(True)
    if error: return miniapp_json_error(error, 401)
    data = miniapp_bot_snapshot(verified["id"])
    return jsonify({"ok": True, "activity": data["activity"], "stats": {"messages": data["messages"], "xp": data["xp"], "level": data["level"], "rank": data["rank"]}})


@web_app.route("/api/miniapp/logout", methods=["POST"])
def miniapp_logout():
    token = request.headers.get("Authorization", "")
    if token.lower().startswith("bearer "): token = token[7:].strip()
    if token:
        with miniapp_db_connect() as conn:
            conn.execute("DELETE FROM app_sessions WHERE token_hash=?", (hashlib.sha256(token.encode()).hexdigest(),))
    response = make_response(jsonify({"ok": True}))
    response.delete_cookie("miniapp_session", path="/")
    return response
...
async def miniapp_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not MINIAPP_URL:
        await update.effective_message.reply_text("⚠️ Mini App URL configured nahi hai. Render mein MINIAPP_URL environment variable set karo.")
        return
    await update.effective_message.reply_text(
        "🚀 𝐒𝐀𝐊𝐒𝐇𝐀𝐌 𝐌𝐈𝐍𝐈 𝐀𝐏𝐏\n\n"
        "✨ Profile • Coins • XP • Rank • Rewards • VIP/Elite\n"
        "🔐 Secure Telegram-bound login system",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🚀 OPEN MINI APP", web_app=WebAppInfo(url=MINIAPP_URL))]])
    )
