...
async def welcome_new_members(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Send the styled welcome message whenever one or more users join a group."""
    if not update.message or not update.message.new_chat_members:
        return

    for member in update.message.new_chat_members:
        # Telegram users always have a first_name; full_name is used as a fallback.
...
async def run_bot():
    ...
    # Welcome message for every new group member.
    app.add_handler(
        MessageHandler(
