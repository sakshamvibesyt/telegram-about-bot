...
def music_keyboard(chat_id):
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("▶️ PLAY", callback_data=f"music_play:{chat_id}"), InlineKeyboardButton("⏸️ PAUSE", callback_data=f"music_pause:{chat_id}")],
        [InlineKeyboardButton("🔀 SHUFFLE", callback_data=f"music_shuffle:{chat_id}"), InlineKeyboardButton("⏭️ NEXT", callback_data=f"music_next:{chat_id}")],
        [InlineKeyboardButton("⏹️ STOP", callback_data=f"music_stop:{chat_id}"), InlineKeyboardButton("📋 QUEUE", callback_data=f"music_queue:{chat_id}")],
        [InlineKeyboardButton("🔁 AUTO PLAY: OFF", callback_data=f"music_auto:{chat_id}")],
    ])

def music_card(track, status="STARTED STREAMING", autoplay=False):
    source = track.get("source_type", "URL")
    return (
        "🎧 <b>" + status + "</b>\n\n"
        "🎵 <b>" + html.escape(track.get("title", "Music")) + "</b>\n"
        "📡 Source: " + html.escape(source) + "\n"
        "👤 Requested by: " + html.escape(track.get("requester", "User")) + "\n\n"
        "00:00 ━━━━━━━━━━━━━━━  LIVE\n\n"
        "🎙️ <b>Assistant</b>\n"
        "🔊 Voice Chat Playback\n"
        f"🔁 AUTO PLAY: {'ON 🟢' if autoplay else 'OFF 🔴'}"
    )

def music_queue_text(chat_id):
    queue = music_queues.get(chat_id, [])
    current = music_current.get(chat_id)
    lines = ["🎵 <b>MUSIC QUEUE</b>", ""]
    if current:
        lines.append(f"▶️ Now: <b>{html.escape(current.get('title','Music'))}</b>")
    else:
        lines.append("⏹️ Nothing is playing.")
    if queue:
        lines.append("\n📋 Up next:")
        for i, item in enumerate(queue[:10], 1):
            lines.append(f"{i}. {html.escape(item.get('title','Music'))}")
    else:
        lines.append("\n📭 Queue is empty.")
    return "\n".join(lines)

async def ensure_music_client():
    global music_client, music_calls, music_started
    if music_started and music_client and music_calls:
        return True, None
    if not TG_API_ID or not TG_API_HASH or not TG_SESSION_STRING:
        return False, "TG_API_ID, TG_API_HASH aur TG_SESSION_STRING Render Environment mein missing hain."
    try:
        from telethon import TelegramClient
        from telethon.sessions import StringSession
        from pytgcalls import PyTgCalls
        music_client = TelegramClient(StringSession(TG_SESSION_STRING), TG_API_ID, TG_API_HASH)
        await music_client.start()
        music_calls = PyTgCalls(music_client)
        await _maybe_await(music_calls.start())
        music_started = True
        print("🎵 Music Assistant + PyTgCalls started.")
        return True, None
    except Exception as e:
        music_client = None
        music_calls = None
        music_started = False
        print(f"⚠️ Music client startup error: {e}")
        return False, str(e)

async def _music_play_source(chat_id, track):
    ok, error = await ensure_music_client()
    if not ok:
        raise RuntimeError(error)
    from pytgcalls.types import MediaStream
    source = track["source"]
    stream = MediaStream(source, video_flags=MediaStream.Flags.IGNORE)
    await _maybe_await(music_calls.play(chat_id, stream))
    music_current[chat_id] = track

async def _music_leave(chat_id):
    if not music_calls:
        return
    method = getattr(music_calls, "leave_call", None) or getattr(music_calls, "leave_group_call", None)
    if method:
        await _maybe_await(method(chat_id))

async def _music_pause(chat_id):
    method = getattr(music_calls, "pause_stream", None) or getattr(music_calls, "pause", None)
    if not method:
        raise RuntimeError("Pause API is not available in this PyTgCalls build.")
    await _maybe_await(method(chat_id))

async def _music_resume(chat_id):
    method = getattr(music_calls, "resume_stream", None) or getattr(music_calls, "resume", None)
    if not method:
        raise RuntimeError("Resume API is not available in this PyTgCalls build.")
    await _maybe_await(method(chat_id))

async def _music_cleanup_file(chat_id):
    path = music_temp_files.pop(chat_id, None)
    if path:
        try:
            os.remove(path)
        except OSError:
...
async def play_command(update, context):
    ...
    if not message or not chat or chat.type not in ("group", "supergroup"):
        if message:
            await message.reply_text("⚠️ /play sirf group voice chat mein use karo.")
        return
    ...
    # Safe source 1: reply to an audio/document already uploaded in Telegram.
    replied = message.reply_to_message
    if replied and (replied.audio or (replied.document and (replied.document.mime_type or "").startswith("audio/"))):
        try:
            file_obj = replied.audio or replied.document
            tg_file = await context.bot.get_file(file_obj.file_id)
            tmp = tempfile.NamedTemporaryFile(prefix="music_", suffix=".mp3", delete=False)
            tmp.close()
            await tg_file.download_to_drive(tmp.name)
            source = tmp.name
            title = file_obj.file_name or "Telegram Audio"
            source_type = "TELEGRAM AUDIO"
            music_temp_files[chat.id] = source
        except Exception as e:
            await message.reply_text(f"❌ Audio download failed: {e}")
            return

    # Safe source 2: a direct HTTP(S) audio/media URL supplied by the user.
    elif query and re.match(r"^https?://\S+$", query, re.I):
        source = query
        title = query.rsplit("/", 1)[-1][:80] or "Remote Audio"
    elif query:
        await message.reply_text(
            "🎵 <b>Music search is not enabled in this build.</b>\n\n"
            "Use /play with a direct audio/media URL, or reply to a Telegram audio file with /play.\n\n"
            "Example: <code>/play https://example.com/song.mp3</code>",
            parse_mode="HTML",
        )
        return
    else:
        await message.reply_text(
            "🎵 <b>How to play</b>\n\n"
            "1️⃣ Reply to an audio file and send <code>/play</code>\n"
            "2️⃣ Or use <code>/play https://.../audio.mp3</code>",
            parse_mode="HTML",
        )
        return
    ...
    track = {
        ...
        "source": source,
        "requester": user.full_name if user else "User",
        "source_type": source_type,
    }
    music_queues.setdefault(chat.id, [])
    if chat.id in music_current:
        music_queues[chat.id].append(track)
        await message.reply_text(
            f"➕ <b>ADDED TO QUEUE</b>\n\n🎵 {html.escape(track['title'])}\n📍 Position: {len(music_queues[chat.id])}",
            parse_mode="HTML",
            reply_markup=music_keyboard(chat.id),
        )
        return

    status_msg = await message.reply_text("🎵 <b>PLEASE WAIT...</b>\n\n👤 Inviting Assistant to the voice chat...", parse_mode="HTML")
    try:
        await _music_play_source(chat.id, track)
        await status_msg.edit_text(music_card(track), parse_mode="HTML", reply_markup=music_keyboard(chat.id))
    except Exception as e:
        await _music_cleanup_file(chat.id)
...
async def music_callback(update, context):
    ...
    if not data.startswith("music_"):
        return
    try:
        action, chat_id_text = data.split(":", 1)
    ...
    try:
        if action == "music_pause":
            await _music_pause(chat_id)
            await query.answer("⏸️ Paused", show_alert=False)
        elif action == "music_play":
            await _music_resume(chat_id)
            await query.answer("▶️ Resumed", show_alert=False)
        elif action == "music_shuffle":
            random.shuffle(music_queues.setdefault(chat_id, []))
            await query.edit_message_text(music_queue_text(chat_id), parse_mode="HTML", reply_markup=music_keyboard(chat_id))
        elif action == "music_queue":
            await query.edit_message_text(music_queue_text(chat_id), parse_mode="HTML", reply_markup=music_keyboard(chat_id))
        elif action == "music_stop":
            await _music_leave(chat_id)
            music_queues[chat_id] = []
            music_current.pop(chat_id, None)
            await _music_cleanup_file(chat_id)
            await query.edit_message_text("⏹️ <b>MUSIC STOPPED</b>\n\n🎙️ Assistant left the voice chat.", parse_mode="HTML")
            ...
            await _music_play_source(chat_id, next_track)
            await query.edit_message_text(music_card(next_track), parse_mode="HTML", reply_markup=music_keyboard(chat_id))
        elif action == "music_auto":
            # UI placeholder for now; automatic end-of-track queue advancing is intentionally off.
            await query.answer("🔁 Auto Play is currently OFF", show_alert=True)
    except Exception as e:
        print(f"⚠️ Music callback error: {e}")
        await query.answer("❌ Music action failed. Check Render logs.", show_alert=True)

async def queue_command(update, context):
    if update.effective_chat:
        await update.effective_message.reply_text(music_queue_text(update.effective_chat.id), parse_mode="HTML", reply_markup=music_keyboard(update.effective_chat.id))

async def pause_command(update, context):
    chat_id = update.effective_chat.id
    try:
        await _music_pause(chat_id)
        await update.effective_message.reply_text("⏸️ <b>MUSIC PAUSED</b>", parse_mode="HTML")
    except Exception as e:
        await update.effective_message.reply_text(f"❌ Pause failed: {html.escape(str(e))}", parse_mode="HTML")

async def resume_command(update, context):
    chat_id = update.effective_chat.id
    try:
        await _music_resume(chat_id)
        await update.effective_message.reply_text("▶️ <b>MUSIC RESUMED</b>", parse_mode="HTML")
    except Exception as e:
        await update.effective_message.reply_text(f"❌ Resume failed: {html.escape(str(e))}", parse_mode="HTML")

async def skip_command(update, context):
    chat_id = update.effective_chat.id
    queue = music_queues.setdefault(chat_id, [])
    if not queue:
        await update.effective_message.reply_text("📭 Queue empty — next track nahi hai.")
        return
    try:
        await _music_cleanup_file(chat_id)
        track = queue.pop(0)
        await _music_play_source(chat_id, track)
        await update.effective_message.reply_text(music_card(track), parse_mode="HTML", reply_markup=music_keyboard(chat_id))
    except Exception as e:
        await update.effective_message.reply_text(f"❌ Skip failed: {html.escape(str(e))}", parse_mode="HTML")
...
async def stop_command(update, context):
    ...
    music_current.pop(chat_id, None)
    await _music_cleanup_file(chat_id)
    await update.effective_message.reply_text("⏹️ <b>MUSIC STOPPED</b>", parse_mode="HTML")
...
async def run_bot():
    ...
    try:
        ...
        try:
            if music_calls:
                stop_method = getattr(music_calls, "stop", None)
                if stop_method:
                    await _maybe_await(stop_method())
        except Exception:
            pass
        try:
            if music_client:
