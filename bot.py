...
# Telegram Mini App / Panel configuration
...
# ==================================
# TELEGRAM PANEL / MINI APP
# ==================================

async def panel_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Open the separate Saksham Panel without changing existing bot commands."""
    user = update.effective_user
    ...
    if not PANEL_URL:
        await update.message.reply_text(
            "⚠️ Panel URL configure nahi hai.\n\n"
            "Owner ko Render environment me PANEL_URL set karna hoga."
        )
        return
    ...
    await update.message.reply_text(
        ...
        "│ 👑 VIP • 💎 Elite • 🛒 Shop\n"
        "╰━━━━━━━━━━━━━━━━━━━━━━━━━━━━╯\n\n"
        "👇 Neeche button se panel open karo.",
        reply_markup=keyboard
    )


def _telegram_init_data_valid(init_data: str, max_age: int = 86400):
...
def _panel_authorized(req):
    """Require the panel's private API key and valid Telegram initData."""
    if not PANEL_API_KEY:
        return None
    supplied_key = req.headers.get("X-Panel-Key", "")
    if not secrets.compare_digest(supplied_key, PANEL_API_KEY):
        return None
    return _telegram_init_data_valid(req.headers.get("X-Telegram-Init-Data", ""))


def _panel_groups(user_id):
    """Return groups in which the bot has stored this user's data."""
    with db_connect() as conn:
        rows = conn.execute(
            """SELECT DISTINCT g.chat_id, g.title
               FROM group_chats g
        ...
        # Include groups that have XP/coins even if group_members was not populated.
        extra = conn.execute(
            """SELECT DISTINCT x.chat_id, COALESCE(g.title, CAST(x.chat_id AS TEXT))
               FROM xp_levels x
               LEFT JOIN group_chats g ON g.chat_id=x.chat_id
              WHERE x.user_id=?""",
            (user_id,)
        ).fetchall()

    merged = {int(cid): (title or str(cid)) for cid, title in rows}
    for cid, title in extra:
        merged.setdefault(int(cid), title or str(cid))
    return [{"chat_id": cid, "title": title} for cid, title in merged.items()]


def _panel_user_data(user_id, chat_id=None):
    groups = _panel_groups(user_id)
    if chat_id is None and groups:
        # Prefer the group with the highest XP as the default profile context.
        with db_connect() as conn:
            row = conn.execute(
                "SELECT chat_id FROM xp_levels WHERE user_id=? ORDER BY xp DESC, updated_at DESC LIMIT 1",
                (user_id,)
            ).fetchone()
        chat_id = int(row[0]) if row else int(groups[0]["chat_id"])
    elif chat_id is not None:
        chat_id = int(chat_id)
        if chat_id not in {g["chat_id"] for g in groups}:
            chat_id = None
    ...
    if chat_id is not None:
        with db_connect() as conn:
            ...
            selected_title = title_row[0] if title_row else ""
            name_row = conn.execute(
                "SELECT name FROM group_members WHERE chat_id=? AND user_id=?",
                (chat_id, user_id)
            ).fetchone()
            selected_name = name_row[0] if name_row else "User"
    ...
    return {
        "user_id": user_id,
        "name": selected_name or "User",
        "username": "",
        "coins": int(total_coins or 0),
        "xp": xp,
        "level": level,
        ...
        "vip": vip,
        "elite": elite,
        "membership": "ELITE" if elite else ("VIP" if vip else "FREE MEMBER"),
        "title": selected_title,
        "referrals": int(referral_count[0]) if referral_count else 0,
        "groups": groups,
        "selected_chat_id": chat_id,
        "activity": [{"event": e, "created_at": d} for e, d in recent],
    }


def _panel_json_error(message, status=400):
...
@web_app.route("/panel-api/me", methods=["GET"])
def panel_api_me():
    tg_user = _panel_authorized(request)
    if not tg_user:
        return _panel_json_error("Unauthorized", 401)
    data = _panel_user_data(int(tg_user["id"]))
    data["username"] = tg_user.get("username", "")
    data["name"] = tg_user.get("first_name") or data["name"]
    data["telegram"] = tg_user
    return jsonify({"ok": True, "user": data})


@web_app.route("/panel-api/groups", methods=["GET"])
def panel_api_groups():
    tg_user = _panel_authorized(request)
    if not tg_user:
        return _panel_json_error("Unauthorized", 401)
    return jsonify({"ok": True, "groups": _panel_groups(int(tg_user["id"]))})


@web_app.route("/panel-api/shop", methods=["GET"])
def panel_api_shop():
    tg_user = _panel_authorized(request)
    if not tg_user:
        return _panel_json_error("Unauthorized", 401)
    ...
    for item_id, name, desc, price, reward_type, reward_value in shop_items():
        data.append({
            ...
            "price": price, "reward_type": reward_type, "reward_value": reward_value
        })
    return jsonify({"ok": True, "items": data, "coins": _panel_user_data(user_id)["coins"]})


@web_app.route("/panel-api/purchase", methods=["POST"])
def panel_api_purchase():
    tg_user = _panel_authorized(request)
    if not tg_user:
        return _panel_json_error("Unauthorized", 401)
    ...
    try:
        ...
        return _panel_json_error("item_id and chat_id are required")

    if chat_id not in {g["chat_id"] for g in _panel_groups(user_id)}:
        return _panel_json_error("Invalid group for this Telegram user", 403)
    ...
    with db_connect() as conn:
        ...
        if not item:
            return _panel_json_error("Shop item not found", 404)
        ...
        if balance < price:
            return _panel_json_error(f"Need {price} coins. You have {balance}.")

        conn.execute(
            "UPDATE coins SET balance=balance-?, updated_at=? WHERE chat_id=? AND user_id=?",
            (price, datetime.utcnow().isoformat(), chat_id, user_id)
        )
        conn.execute(
            "INSERT INTO coin_purchases(chat_id,user_id,item_id,item_name,price,status,created_at) VALUES(?,?,?,?,?,?,?)",
            (chat_id, user_id, item_id, name, price, "claimed", datetime.utcnow().isoformat())
        )
        ...
        if reward_type == "xp":
            ...
            old_xp = int(current[0]) if current else 0
            old_level = int(current[1]) if current else 1
            new_xp = old_xp + int(reward_value)
            # Keep the bot's existing level formula by calculating thresholds.
            new_level = old_level
            try:
                while new_level < 100 and new_xp >= xp_for_level(new_level):
                    new_level += 1
            except Exception:
                pass
            now = datetime.utcnow().isoformat()
            if current:
                conn.execute(
                    "UPDATE xp_levels SET xp=?,level=?,updated_at=? WHERE chat_id=? AND user_id=?",
                    (new_xp, new_level, now, chat_id, user_id)
                )
            else:
                conn.execute(
                    "INSERT INTO xp_levels(chat_id,user_id,xp,level,message_count,updated_at) VALUES(?,?,?,?,?,?)",
                    (chat_id, user_id, new_xp, new_level, 0, now)
                )
        elif reward_type == "coins":
            conn.execute(
                "UPDATE coins SET balance=balance+?, updated_at=? WHERE chat_id=? AND user_id=?",
                (int(reward_value), datetime.utcnow().isoformat(), chat_id, user_id)
            )

    return jsonify({"ok": True, "message": f"{name} claimed successfully", "user": _panel_user_data(user_id, chat_id)})
