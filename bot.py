...
def _voice_language(text):
    """Use Hindi TTS for Devanagari; English is the fallback for Roman/Hinglish."""
...
async def send_voice_from_text(update, context):
    ...
    try:
        ...
        # Natural Microsoft neural female voice for Hindi/Hinglish.
        # English-heavy text is also handled with an Indian English neural voice.
        devanagari = sum("\u0900" <= ch <= "\u097F" for ch in text_to_speak)
        latin_letters = sum(ch.isascii() and ch.isalpha() for ch in text_to_speak)
        hindi_script_ratio = devanagari / max(1, len(text_to_speak))

        if devanagari > 0 or hindi_script_ratio > 0.02:
            voice_name = "hi-IN-SwaraNeural"
        else:
            # Indian English female neural voice for English-heavy messages.
        ...
        await message.reply_voice(
            voice=audio,
            caption="🎙️ Natural AI Voice • Female"
        )

    except Exception as e:
        print(f"⚠️ Neural voice generation error: {e}")
        await message.reply_text(
            "❌ Natural voice generate nahi ho payi. "
            "Render logs check karo aur <code>edge-tts</code> installed hai ya nahi verify karo.",
            parse_mode="HTML",
        )
