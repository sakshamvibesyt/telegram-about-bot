...
async def tagall_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Tag each known member in a separate message.

    Usage: /tagall [optional message]
    Example: /tagall GOOD EVENING GUYS KI HAL CHAL 🥰

    Each member receives a separate message containing the custom intro
    followed by that member's clickable mention.
    """
    chat = update.effective_chat
    user = update.effective_user
    ...
    # Keep mass-tagging admin controlled.
    try:
        member = await context.bot.get_chat_member(chat.id, user.id)
        if member.status not in ("creator", "administrator") and user.id != OWNER_ID:
            await update.message.reply_text(
        ...
        if user.id != OWNER_ID:
            await update.message.reply_text(
                "❌ Admin permission verify nahi ho saki."
            )
            return

    if chat.id in tag_running:
        await update.message.reply_text(
            "⏳ Tagging already chal rahi hai. Please wait..."
        )
        return

    members = group_members(chat.id)
    # Do not tag the person who started the command.
    members = [(uid, name) for uid, name in members if uid != user.id]
    ...
    try:
        ...
        for uid, name in members:
            ...
            try:
                ...
                print(f"⚠️ Could not tag {uid}: {e}")
                # Continue with the remaining members instead of stopping
                # the complete /tagall run.
                continue

            # One message per member, with a small delay to reduce
            # Telegram flood-limit problems.
            await asyncio.sleep(TAG_DELAY)
        ...
        await context.bot.send_message(
            ...
            text=(
                f"✅ 𝐓𝐀𝐆𝐆𝐈𝐍𝐆 𝐂𝐎𝐌𝐏𝐋𝐄𝐓𝐄\n\n"
                f"👥 Separate messages sent: {sent}/{total}"
            ),
        )

    except Exception as e:
        print(f"⚠️ Separate tagall error: {e}")
        try:
            await context.bot.send_message(
                chat_id=chat.id,
                text=(
                    "⚠️ /tagall stop ho gayi. "
                    "Telegram limit/permission ki wajah se ho sakta hai."
                ),
            )
        except Exception:
            pass
