...
async def send_voice_from_text(update, context):
    ...
    try:
        ...
        gTTS(
            text=text_to_speak,
            lang=_voice_language(text_to_speak),
            slow=False
        ).write_to_fp(audio)
        audio.seek(0)
        await message.reply_voice(voice=audio, caption="🎙️ Voice by Bot")
    except Exception as e:
        print(f"⚠️ Voice generation error: {e}")
        await message.reply_text(
            "❌ Voice generate nahi ho payi. Render me <code>gTTS</code> install karo.",
            parse_mode="HTML",
        )
