...
# Telegram Mini App public URL. Set this in Render after deployment.
...
# Create this ONCE before any Mini App/API decorators are registered.
# Re-creating Flask here later would discard all previously registered routes.
...
async def miniapp_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Open the separate Telegram Mini App without changing existing bot flows."""
    if not update.message:
        return

    if not MINIAPP_URL:
        await update.message.reply_text(
            "⚠️ Mini App abhi configure nahi hai.\n\n"
            "Owner ko Render Environment mein MINIAPP_URL set karna hoga."
        )
        return
    ...
    await update.message.reply_text(
        ...
        "🔐 Login / Register inside the Mini App.\n\n"
...
# ==================================
# MINI APP AUTH + DATA API (STEP 2)
# ==================================

MINIAPP_DB_FILE = os.environ.get("MINIAPP_DB_FILE", "miniapp_data.db").strip()
MINIAPP_SESSION_DAYS = 7
...
def init_miniapp_db():
    with miniapp_db_connect() as conn:
        conn.execute(
            "CREATE TABLE IF NOT EXISTS miniapp_accounts ("
        ...
        conn.execute("CREATE INDEX IF NOT EXISTS idx_miniapp_sessions_user ON miniapp_sessions(telegram_id)")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_miniapp_sessions_expiry ON miniapp_sessions(expires_at)")
...
def _miniapp_password_hash(password, salt_hex):
    import hashlib
...
def _miniapp_make_password_hash(password):
    salt_hex = secrets.token_bytes(16).hex()
    return salt_hex, _miniapp_password_hash(password, salt_hex)


def _miniapp_valid_username(value):
    return bool(re.fullmatch(r"[A-Za-z0-9_]{3,32}", value or ""))


def _miniapp_verify_init_data(init_data):
    """Verify Telegram Mini App initData on the server; never trust initDataUnsafe."""
    if not init_data:
        raise ValueError("Telegram initData missing. Open the app from Telegram.")
    ...
    try:
        ...
        raise ValueError("Invalid Telegram initData.") from exc

    if not received_hash or not telegram_id or not auth_date:
        raise ValueError("Invalid Telegram authentication data.")

    # Reject very old initData so a captured payload cannot be reused indefinitely.
    if abs(int(time.time()) - auth_date) > 86400:
        raise ValueError("Telegram authentication data has expired. Reopen the Mini App.")
    ...
    if not hmac.compare_digest(calculated_hash, received_hash):
        raise ValueError("Telegram authentication verification failed.")
    ...
    return {
        ...
        "name": " ".join(x for x in [user.get("first_name"), user.get("last_name")] if x).strip() or "Telegram User",
    }


def _miniapp_init_data_from_request():
    return request.headers.get("X-Telegram-Init-Data", "").strip()


def _miniapp_session_from_request(telegram_user):
    auth = request.headers.get("Authorization", "")
    if not auth.startswith("Bearer "):
        raise ValueError("Mini App session missing. Please login again.")
    token = auth[7:].strip()
    if not token:
        raise ValueError("Mini App session missing. Please login again.")

    now = _miniapp_now().isoformat()
    with miniapp_db_connect() as conn:
        row = conn.execute(
            "SELECT telegram_id FROM miniapp_sessions WHERE token=? AND expires_at>?",
            (token, now),
        ).fetchone()
        if not row or int(row[0]) != int(telegram_user["id"]):
            raise ValueError("Session expired or invalid. Please login again.")
    return token


def _miniapp_json_error(message, status=400):
    return jsonify({"ok": False, "error": message}), status


def _miniapp_auth_user():
    try:
        telegram_user = _miniapp_verify_init_data(_miniapp_init_data_from_request())
        _miniapp_session_from_request(telegram_user)
        return telegram_user, None
    except ValueError as exc:
        return None, _miniapp_json_error(str(exc), 401)


def _miniapp_register_or_login_status(telegram_user):
    with miniapp_db_connect() as conn:
        row = conn.execute(
            "SELECT username FROM miniapp_accounts WHERE telegram_id=?",
            (telegram_user["id"],),
        ).fetchone()
    return row[0] if row else None


def _miniapp_create_session(telegram_id):
    token = secrets.token_urlsafe(48)
    now = _miniapp_now()
    expires = now + timedelta(days=MINIAPP_SESSION_DAYS)
    with miniapp_db_connect() as conn:
        conn.execute(
            "INSERT INTO miniapp_sessions(token,telegram_id,created_at,expires_at) VALUES(?,?,?,?)",
            (token, telegram_id, now.isoformat(), expires.isoformat()),
        )
        # Keep the separate Mini App DB small and remove old sessions for this account.
        conn.execute(
            "DELETE FROM miniapp_sessions WHERE telegram_id=? AND expires_at<=?",
            (telegram_id, now.isoformat()),
        )
    return token


def _miniapp_group_list(user_id):
    """Return groups in which this user has existing bot-side data."""
    with db_connect() as conn:
        rows = conn.execute(
            "SELECT DISTINCT chat_id FROM ("
            "SELECT chat_id FROM coins WHERE user_id=? "
            "UNION SELECT chat_id FROM xp_levels WHERE user_id=? "
            "UNION SELECT chat_id FROM group_members WHERE user_id=? "
            "UNION SELECT chat_id FROM vip_users WHERE user_id=? "
            "UNION SELECT chat_id FROM elite_users WHERE user_id=?"
            ")",
        ...
        for (chat_id,) in rows:
            title_row = conn.execute(
                "SELECT title FROM group_chats WHERE chat_id=?",
                (chat_id,),
            ).fetchone()
            if title_row and title_row[0]:
                ...
                member_row = conn.execute(
                    "SELECT name FROM group_members WHERE chat_id=? AND user_id=?",
                    (chat_id, user_id),
                ).fetchone()
                title = f"Group {chat_id}" if not member_row else f"Group {chat_id}"
            latest = conn.execute(
                "SELECT MAX(ts) FROM ("
                "SELECT updated_at AS ts FROM coins WHERE chat_id=? AND user_id=? "
                "UNION ALL SELECT updated_at FROM xp_levels WHERE chat_id=? AND user_id=? "
                "UNION ALL SELECT updated_at FROM group_members WHERE chat_id=? AND user_id=?"
                ")",
                (chat_id, user_id, chat_id, user_id, chat_id, user_id),
            ).fetchone()[0] or ""
            result.append({"chat_id": int(chat_id), "title": title, "latest": latest})
    result.sort(key=lambda x: x["latest"], reverse=True)
    return result


def _miniapp_group_data(user_id, chat_id):
    groups = _miniapp_group_list(user_id)
    allowed = {g["chat_id"] for g in groups}
    if chat_id is None:
        chat_id = groups[0]["chat_id"] if groups else None
    else:
        try:
            chat_id = int(chat_id)
        except Exception:
        ...
        if chat_id not in allowed:
            raise ValueError("This group is not available for your Telegram account.")

    if chat_id is None:
        return {
            "chat_id": None,
            "group_name": "No group data yet",
            "coins": 0,
            "xp": 0,
            "level": 1,
            "messages": 0,
    ...
    with db_connect() as conn:
        ...
        xp_row = conn.execute(
            "SELECT xp,level,message_count FROM xp_levels WHERE chat_id=? AND user_id=?",
            (chat_id, user_id),
        ).fetchone()
        vip = bool(conn.execute(
            "SELECT 1 FROM vip_users WHERE chat_id=? AND user_id=?",
            (chat_id, user_id),
        ).fetchone())
        elite = bool(conn.execute(
            "SELECT 1 FROM elite_users WHERE chat_id=? AND user_id=?",
            (chat_id, user_id),
        ).fetchone())
        title_row = conn.execute(
            "SELECT title FROM custom_titles WHERE chat_id=? AND user_id=?",
            (chat_id, user_id),
        ).fetchone()
        ...
        xp, level, messages = xp_row if xp_row else (0, 1, 0)
        balance = int(coin_row[0]) if coin_row else 0
        rank_row = conn.execute(
            "SELECT COUNT(*) + 1 FROM xp_levels WHERE chat_id=? AND xp>?",
            (chat_id, xp),
        ).fetchone() if xp_row else None
        rank = int(rank_row[0]) if rank_row else None

    status = "ELITE" if elite else ("VIP" if vip else "MEMBER")
    display_title = title_row[0] if title_row and title_row[0] else status
    return {
        "chat_id": chat_id,
        "group_name": (group_row[0] if group_row and group_row[0] else f"Group {chat_id}"),
        "coins": balance,
        "xp": int(xp),
        "level": int(level),
...
def _miniapp_dashboard(telegram_user, chat_id=None):
    data = _miniapp_group_data(telegram_user["id"], chat_id)
    with db_connect() as conn:
        total_coins = conn.execute(
            "SELECT COALESCE(SUM(balance),0) FROM coins WHERE user_id=?",
            (telegram_user["id"],),
        ).fetchone()[0]
    data["total_coins"] = int(total_coins or 0)
    data["telegram"] = {
        "id": telegram_user["id"],
...
@web_app.route("/api/miniapp/status", methods=["GET"])
def miniapp_status_api():
    try:
        telegram_user = _miniapp_verify_init_data(_miniapp_init_data_from_request())
        username = _miniapp_register_or_login_status(telegram_user)
        return jsonify({
            "ok": True,
            "registered": bool(username),
            "app_username": username or "",
            "telegram": telegram_user,
        })
    except ValueError as exc:
        return _miniapp_json_error(str(exc), 401)


@web_app.route("/api/miniapp/register", methods=["POST"])
def miniapp_register_api():
    try:
        telegram_user = _miniapp_verify_init_data(_miniapp_init_data_from_request())
        ...
        if not _miniapp_valid_username(username):
            return _miniapp_json_error("Username 3-32 characters ka ho aur sirf letters, numbers, underscore use kare.")
        if len(password) < 8:
            return _miniapp_json_error("Password minimum 8 characters ka hona chahiye.")
        if password != confirm:
            return _miniapp_json_error("Passwords do not match.")

        salt_hex, password_hash = _miniapp_make_password_hash(password)
        now = _miniapp_now().isoformat()
        with miniapp_db_connect() as conn:
            ...
            if existing_telegram:
                return _miniapp_json_error("Is Telegram account ka Mini App account already registered hai. Login karo.", 409)
            existing_username = conn.execute(
                "SELECT telegram_id FROM miniapp_accounts WHERE username=? COLLATE NOCASE",
                (username,),
            ).fetchone()
            if existing_username:
                return _miniapp_json_error("Ye Mini App username already taken hai.", 409)
            conn.execute(
                "INSERT INTO miniapp_accounts(telegram_id,username,password_hash,salt,created_at,updated_at) VALUES(?,?,?,?,?,?)",
                (telegram_user["id"], username, password_hash, salt_hex, now, now),
            )
        token = _miniapp_create_session(telegram_user["id"])
        return jsonify({"ok": True, "session": token, "username": username})
    except ValueError as exc:
        return _miniapp_json_error(str(exc), 401)
    except sqlite3.IntegrityError:
        return _miniapp_json_error("Username already taken. Try another username.", 409)
    except Exception as exc:
        print(f"⚠️ Mini App register error: {exc}")
        return _miniapp_json_error("Registration failed. Please try again.", 500)


@web_app.route("/api/miniapp/login", methods=["POST"])
def miniapp_login_api():
    try:
        telegram_user = _miniapp_verify_init_data(_miniapp_init_data_from_request())
        body = request.get_json(silent=True) or {}
        username = str(body.get("username", "")).strip()
        ...
        if not username or not password:
            return _miniapp_json_error("Username aur password required hai.")

        with miniapp_db_connect() as conn:
            row = conn.execute(
                "SELECT telegram_id,password_hash,salt,username FROM miniapp_accounts WHERE username=? COLLATE NOCASE",
                (username,),
            ).fetchone()
        if not row:
            return _miniapp_json_error("Invalid username ya password.", 401)
        if int(row[0]) != int(telegram_user["id"]):
            return _miniapp_json_error("Ye account kisi doosre Telegram account se bound hai.", 403)
        calculated = _miniapp_password_hash(password, row[2])
        import hmac
        if not hmac.compare_digest(calculated, row[1]):
            return _miniapp_json_error("Invalid username ya password.", 401)

        token = _miniapp_create_session(telegram_user["id"])
        with miniapp_db_connect() as conn:
            conn.execute(
                "UPDATE miniapp_accounts SET updated_at=? WHERE telegram_id=?",
                (_miniapp_now().isoformat(), telegram_user["id"]),
            )
        return jsonify({"ok": True, "session": token, "username": row[3]})
    except ValueError as exc:
        return _miniapp_json_error(str(exc), 401)
    except Exception as exc:
        print(f"⚠️ Mini App login error: {exc}")
        return _miniapp_json_error("Login failed. Please try again.", 500)


@web_app.route("/api/miniapp/dashboard", methods=["GET"])
def miniapp_dashboard_api():
    telegram_user, error = _miniapp_auth_user()
    if error:
        return error
    try:
        chat_id = request.args.get("chat_id")
        data = _miniapp_dashboard(telegram_user, chat_id)
        return jsonify({"ok": True, "data": data})
    except ValueError as exc:
        return _miniapp_json_error(str(exc), 403)
    except Exception as exc:
        print(f"⚠️ Mini App dashboard error: {exc}")
        return _miniapp_json_error("Dashboard data load nahi ho paya.", 500)


@web_app.route("/api/miniapp/logout", methods=["POST"])
def miniapp_logout_api():
    telegram_user, error = _miniapp_auth_user()
    if error:
        return error
    auth = request.headers.get("Authorization", "")
    ...
    with miniapp_db_connect() as conn:
        conn.execute("DELETE FROM miniapp_sessions WHERE token=? AND telegram_id=?", (token, telegram_user["id"]))
...
def miniapp_page():
    """Serve the Mini App frontend from the repository."""
