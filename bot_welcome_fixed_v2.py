...
async def welcome_chat_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Welcome users when Telegram reports a new member/subscriber."""
    ...
    previous_statuses = {"left", "kicked"}

    if new_status not in joined_statuses or old_status not in previous_statuses:
        return

    user = cm.new_chat_member.user
    name = user.first_name or user.full_name or "Friend"
...
╰━━━━━━━━━━━━━━━━━━━━━━╯"""
    ...
    try:
        ...
        print(f"👋 Welcome sent to {name} in {update.effective_chat.id}")
    except Exception as e:
        print(f"⚠️ Welcome message failed: {e}")
...
async def run_bot():
    ...
    # Also handle chat-member updates (useful for channels/subscribers).
    app.add_handler(
