...
/id — Your Telegram ID
/ping — Bot status
/movie <name> — Movie search

👇 Buttons se bhi bot explore kar sakte ho!""",
            reply_markup=back_button()
        )

    elif query.data == "movie_example":
        await query.answer()
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton(
                "🔎 SEARCH INTERSTELLAR",
                url="https://www.google.com/search?q=Interstellar+movie"
            )],
            [InlineKeyboardButton(
                "🎬 TMDB",
                url="https://www.themoviedb.org/search/movie?query=Interstellar"
            )],
            [InlineKeyboardButton(
                "📺 JUSTWATCH",
                url="https://www.justwatch.com/in/search?q=Interstellar"
            )],
        ...
        await query.edit_message_text(
            ...
            "👇 Search/details ke liye option choose karo:",
            reply_markup=keyboard,
...
# ==================================
# MOVIE SEARCH (NO API REQUIRED)
# ==================================

async def movie_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Search for a movie using public web search pages, without an API.

    This does not fetch or redistribute copyrighted movie files. It gives the
    user useful search buttons for movie information and legal availability.
    """
    save_user(update.effective_user.id)

    if not context.args:
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("🎬 TRY EXAMPLE", callback_data="movie_example")],
            [InlineKeyboardButton("🔙 MAIN MENU", callback_data="menu")],
        ])
        await update.message.reply_text(
            "🎬 𝐌𝐎𝐕𝐈𝐄 𝐒𝐄𝐀𝐑𝐂𝐇\n\n"
            "Movie name ke saath command use karo:\n\n"
            "👉 /movie Interstellar\n"
            "👉 /movie Avengers Endgame\n\n"
            "⚡ API ki zarurat nahi hai.",
            reply_markup=keyboard,
        )
        return
    ...
    if len(movie_name) > 150:
        await update.message.reply_text(
            "❌ Movie name bahut lamba hai.\n"
            "Please 150 characters ke andar movie name bhejo."
        )
        return

    if not movie_name:
        await update.message.reply_text(
            "❌ Movie name missing hai.\n\nExample: /movie Interstellar"
        )
        return

    encoded = quote_plus(movie_name)

    keyboard = InlineKeyboardMarkup([
        [
            InlineKeyboardButton(
                "🔎 GOOGLE",
                url=f"https://www.google.com/search?q={encoded}+movie"
            ),
            InlineKeyboardButton(
                "🎬 TMDB",
                url=f"https://www.themoviedb.org/search/movie?query={encoded}"
            ),
        ],
        [
            InlineKeyboardButton(
                "📺 JUSTWATCH",
                url=f"https://www.justwatch.com/in/search?q={encoded}"
            )
        ],
        [
            InlineKeyboardButton("🔙 MAIN MENU", callback_data="menu")
    ...
    try:
        await update.message.reply_text(
            f"🎬 𝐌𝐎𝐕𝐈𝐄 𝐒𝐄𝐀𝐑𝐂𝐇\n\n"
            f"🔍 Search: {movie_name}\n\n"
            "👇 Neeche se movie details/availability check karo:",
            reply_markup=keyboard,
            disable_web_page_preview=True,
        )
    except Exception as e:
        print(f"⚠️ Movie command error: {e}")
        try:
            await update.message.reply_text(
                "❌ Movie search mein error aa gaya. Please dobara try karo."
            )
        except Exception:
            pass
...
async def set_promo_group(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "📢 Ab har 10 minute mein JOIN MY CHANNELS message bheja jayega."
    )
